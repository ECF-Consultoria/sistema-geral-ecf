...
class DesempenhoMetadadosCacheTest extends TestCase
{
    ...
    public function test_compute_cached_grava_sob_v6_com_period_key_mes_fechado(): void
    {
        ...
        $this->assertSame('desempenho.compute.v14.' . $user->id . '.2026-06', $chaveEsperada,
            'mês fechado usa period_key=YYYY-MM na chave (Fase 120: bump v13→v14, AGRE-03).');
    ...
    public function test_cache_key_operacional_nao_colide_com_chave_de_mes_fechado_do_mesmo_mes(): void
    {
        ...
        $chaveFechadaHipotetica = sprintf('desempenho.compute.v14.%d.%s', $userId, $mes->format('Y-m'));

        $this->assertSame('desempenho.compute.v14.999.current_month', $chaveOperacional,
            'mês corrente usa period_key=current_month — nunca YYYY-MM (Fase 120: bump v13→v14, AGRE-03).');
    ...
    public function test_cache_key_helper_bate_com_a_chave_realmente_gravada_por_compute_cached(): void
    {
        ...
        $this->assertSame('desempenho.compute.v14.' . $user->id . '.current_month', $chaveHelper);

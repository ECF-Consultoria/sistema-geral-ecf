...
class DesempenhoMetadadosCacheTest extends TestCase
{
    ...
    public function test_compute_cached_grava_sob_v6_com_period_key_mes_fechado(): void
    {
        ...
        $this->assertSame('desempenho.compute.v13.' . $user->id . '.2026-06', $chaveEsperada,
            'mês fechado usa period_key=YYYY-MM na chave (Fase 116: bump v11→v12, NPSFLOOR).');
    ...
    public function test_cache_key_operacional_nao_colide_com_chave_de_mes_fechado_do_mesmo_mes(): void
    {
        ...
        $chaveFechadaHipotetica = sprintf('desempenho.compute.v13.%d.%s', $userId, $mes->format('Y-m'));

        $this->assertSame('desempenho.compute.v13.999.current_month', $chaveOperacional,
            'mês corrente usa period_key=current_month — nunca YYYY-MM (Fase 116: bump v11→v12, NPSFLOOR).');
    ...
    public function test_cache_key_helper_bate_com_a_chave_realmente_gravada_por_compute_cached(): void
    {
        ...
        $this->assertSame('desempenho.compute.v13.' . $user->id . '.current_month', $chaveHelper);
    ...
    #[Test]
    public function test_user_so_shopee_nao_fica_mais_blocked_score_partial_com_margem_placeholder(): void
    {
        Carbon::setTestNow(Carbon::parse('2026-07-20 14:00:00'));
        ...
        // PASSA a ser financeiramente elegível (financial_metrics_eligible
        // vem de CarteiraContextService a partir do setor 'shopee') e a
        // margem das empresas Shopee vira placeholder=1.0 (piso da régua).
        // Este cenário SUPERA o teste anterior (v17), que esperava 'blocked'.
        // Sem nenhuma linha de shopee_metrics no fixture, var_faturamento_pct
        // fica sem baseline (null) → score_status vira 'partial' (não mais
        // 'blocked') — e a margem placeholder sozinha já sustenta a nota.
    ...
    public function test_user_misto_performance_e_shopee_produz_score_unico_official(): void
    {
        ...
        $service = app(DesempenhoScoreService::class);
        // Sem HTTP fixture de faturamento/margem (nem shopee_metrics) — var_*
        // ficam null (sem baseline nas duas empresas), então score_status vira
        // 'partial' (não 'blocked', pois há 2 vínculos financeiros elegíveis:
        ...
        // (financial_metrics_eligible=true) — antes só a empresa Performance
        // contava (1); agora as 2 contam.

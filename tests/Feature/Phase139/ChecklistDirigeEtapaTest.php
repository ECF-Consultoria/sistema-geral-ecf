 ...
 * prova de que os caminhos HTTP disparam a sincronização é do plano 139-08
 * (`ChecklistEndpointsTest`).
 *
 * Setup copiado de `FinalizarTravaTest.php`/`FinalizarTransicaoEtapaTest.php`
 * (Fase 139 Plano 06) — mesma blindagem de `Http::fake()`/`Queue::fake()`/
...
class ChecklistDirigeEtapaTest extends TestCase
{
    ...
    private function servicoComContrato(): Servico
    {
        return Servico::create([
            'nome'           => 'Gestão de Tráfego (sincronizador 139-07)',
            'valor_padrao'   => 100,
    ...
    {
        return Servico::create([
            'nome'           => 'Polos (sincronizador 139-07)',
            'valor_padrao'   => 100,
            'tipo_cobranca'  => Servico::TIPO_MENSAL,
            'ativo'          => true,
    ...
    private function completarGrupoEntrada(Company $empresa, User $usuario, array $manuaisAIgnorar = []): void
    {
        ...
        OnboardingLink::create(['company_id' => $empresa->id, 'token' => 'token-sincronizador-139-07-' . $empresa->id]);
    }

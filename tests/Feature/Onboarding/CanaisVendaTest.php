...
class CanaisVendaTest extends TestCase
{
    ...
    public function test_padrao_do_item_tem_as_chaves_novas_sem_perder_valor(): void
    {
        ...
        $this->assertSame(['canal' => '', 'outro' => '', 'valor' => '', 'feito' => false], $padrao);
    }
    ...
    {
        return [
            'nada respondido'          => [['canal' => '', 'outro' => '', 'valor' => ''], false],
            'não vende fora do ML'     => [['canal' => 'Não vendo em outros canais', 'valor' => ''], true],
            'canal sem faixa'          => [['canal' => 'Shopee', 'valor' => ''], false],
            'canal com faixa'          => [['canal' => 'Shopee', 'valor' => 'De 50 a 100k'], true],
            'outro sem nome do canal'  => [['canal' => 'Outro', 'outro' => '', 'valor' => 'Até 50k'], false],
            'outro com nome do canal'  => [['canal' => 'Outro', 'outro' => 'Shein', 'valor' => 'Até 50k'], true],
            'faixa sem canal'          => [['canal' => '', 'valor' => 'Acima de 500k'], false],
        ];
    }

    // ─── Gravação pela rota pública ──────────────────────────────────────────

    public function test_cliente_responde_canal_e_faixa(): void
    {
        $impl = $this->criarImpl();

        $this->salvar($impl, 'canal', 'Madeira Madeira')->assertOk();
        ...
        $this->assertSame('Madeira Madeira', $dado['canal']);
    ...
    public function test_quem_nao_vende_em_outros_canais_fecha_o_item_sem_faixa(): void
    {
        ...
        $this->salvar($impl, 'canal', MlbImplementacao::CANAL_NENHUM)->assertOk();
        $this->salvar($impl, 'feito', true)->assertOk();
    ...
    {
        // Trava anti-check-vazio no servidor: escolher o canal e parar ali não fecha o item.
        $impl = $this->criarImpl();

        $this->salvar($impl, 'canal', 'Shopee')->assertOk();
    ...
    {
        return [
            'canal e faixa'       => [['canal' => 'Shopee', 'valor' => 'De 50 a 100k'], 'Shopee · De 50 a 100k'],
            'outro canal'         => [['canal' => 'Outro', 'outro' => 'Shein', 'valor' => 'Até 50k'], 'Outro: Shein · Até 50k'],
            'não vende fora'      => [['canal' => 'Não vendo em outros canais'], 'Não vendo em outros canais'],
            // Ficha que respondeu quando o item só tinha a faixa continua legível.
            'só a faixa (antiga)' => [['valor' => 'Acima de 500k'], 'Acima de 500k'],
            'canal sem faixa'     => [['canal' => 'Magalu'], 'Magalu'],
    ...
    public function test_faixa_nao_acompanha_quem_disse_que_nao_vende(): void
    {
        ...
        $dados['itens']['canais_faturamento'] = [
            'canal' => MlbImplementacao::CANAL_NENHUM,
            'outro' => '',
            'valor' => 'Acima de 500k',
            'feito' => true,
        ];
    ...
    public function test_ficha_salva_antes_da_pergunta_consegue_responder(): void
    {
        ...
        $this->salvar($impl, 'canal', 'Magalu')->assertOk();
        $this->salvar($impl, 'valor', 'Até 50k')->assertOk();

        $dado = $impl->refresh()->dados['itens']['canais_faturamento'];

        $this->assertSame('Magalu', $dado['canal']);
        $this->assertSame('Até 50k', $dado['valor']);
    }

    public function test_faixa_ja_respondida_sobrevive_a_chegada_da_pergunta_do_canal(): void
    {
        // A faixa continua em `valor`: quem respondeu quando o item era só a faixa não
        // perde a resposta ao ganhar a chave `canal`.
        $antigo = MlbImplementacao::dadosPadrao();

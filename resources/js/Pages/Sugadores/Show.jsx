...
export default function SugadoresShow({
    ...
    can_update,
    mlbs = [],
    // Phase 54 (A2): props `sugador_config` e `can_manage_config` deixadas
    // de ser consumidas — backend `SugadorController::show` continua enviando
    // (sem quebrar), mas o Show foca só no sugador único. Cleanup do backend
    // vira seed futuro (props ignoradas pelo React sem warning).
}) {
    ...
    return (
        <AppLayout title="Sugador">
            ...
            {/* Phase 54 (A2): ConfigResumoCard removido (vivia aqui na Wave 3 da
                Phase 52 mas view individual é sobre 1 sugador — config é per-empresa
                e agora vive apenas em EmpresaListagem). Para tipo=adgroup, MlbsDoAdgroup
                ocupa a linha inteira; para tipo=campanha, nada substitui. */}
            {sugador.tipo === 'adgroup' && (
                <MlbsDoAdgroup
...
// Phase 54 (A2): componente local ConfigResumoCard removido — a definicao
// canonica vive em EmpresaListagem.jsx (drilldown per-empresa). Show.jsx nao
// precisa mais dele porque a view individual e' sobre 1 sugador.

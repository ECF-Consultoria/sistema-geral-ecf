...
**`.planning/learnings/painel-polos-desempenho-da-grade.md`** — leitura obrigatória antes de mexer na grade do Painel Polos (ou em qualquer grade editável inline). Por que o gargalo era o tamanho do DOM e não a query, por que `<option>` sob demanda exige `flushSync`, as cinco props que furavam o `memo()` da linha, e por que virtualização ficou de fora de propósito.

**`.planning/learnings/sync-da-planilha-de-polos.md`** — leitura obrigatória antes de rodar `polos:sync-planilha` contra produção. Por que a ordem é **reconciliar → sync** (sem isso o painel duplica: 38 duplicatas em 31/07, 13 em 26/08) e por que `--arquivar-ausentes` não pega essas duplicatas; conferir o CABEÇALHO antes de tudo (a coluna `Fase` já chegou renomeada para `Loja 2`, e coluna que não casa some do relatório em silêncio); as sujeiras recorrentes da planilha que não são bugs do código; e a receita de deploy por worktree — o `main` local costuma estar centenas de commits atrás do `origin/main`, e commitar por caminho arrasta o trabalho da outra sessão.

**`.planning/learnings/fechamento-tabela-por-empresa.md`** — leitura obrigatória antes de mexer em tabela progressiva, faixa de faturamento ou mensalidade do fechamento (Fases 137-141). Por que a tabela do serviço deixou de ser régua e virou modelo de partida/semente da materialização; por que `origem='presumida_servico'` nunca conta como confirmada; a ordem obrigatória materializar → conferir → comparar → ligar (inverter esvazia o fechamento); como desligar a chave sem deploy; por que competência congelada não muda sozinha com a virada; a armadilha do gate de cobertura com `valor_fixo`; e os números literais medidos na virada em produção de 2026-09-09/10 (queda de R$ 1,75 milhão no total a receber, confirmada pelo usuário como correção do "antes" que estava errado).

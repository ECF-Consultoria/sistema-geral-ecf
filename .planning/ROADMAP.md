...
**Plans:** 3 plans em 1 wave (paralelos — sem sobreposição de arquivos). Auditoria + gate das suítes já escritas nas Fases 112-114 (não é criação do zero) + 1 suíte nova de invariantes + doc da regra.

Plans:
- [ ] 115-01-PLAN.md — Auditoria + gate das suítes nucleares: resolver (6 casos), enriquecimento e dedup [HUB-TEST-01, HUB-TEST-02, HUB-TEST-03]
- [ ] 115-02-PLAN.md — Auditoria replay + listagem + suíte nova de invariantes transversais (guarda anti-rede-real + tokens fora do log) [HUB-TEST-04, HUB-TEST-05]

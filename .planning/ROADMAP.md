...
- [x] **Phase 46: Histórico longitudinal de scores na página de desempenho** — snapshot diário em `desempenho_score_snapshots` via comando `desempenho:snapshot-scores` agendado às 13:30 BRT (depois da cascata D-1). UI mostra micro-deltas `↑ +2.3 hoje · ↑ +5.1 sem.` no ranking + EvolucaoDrawer com gráfico Recharts (linha do user + linha pontilhada da mediana do grupo). Endpoint `GET /api/performance/{user}/evolucao`. Insight do research: metodologia justa do briefing já estava implementada em PortfolioScoreService (quick 260623); Phase 46 só persiste + compara + visualiza.
- [ ] **Phase 47: Scoring por função com balanceamento por volume** *(v12.0 audit 2026-07-03: CONGELADA para retomada futura — depende de Phase 44 que está BLOCKED em checkpoint humano; não impede fechamento de v12.0)* — adiciona pontuação `sugador-resolvido-via-sistema` (analistas, **scoring negativo por não-resolvido**) + `PPA-concluído` (estrategistas). **Balanceamento crítico:** sugador é diário (recompensa pequena ou penalização) vs PPA mensal (recompensa proporcionalmente maior). Pesos por volume esperado. Depends on Phase 44 + Phase 46.
- [x] **Phase 48: Redesign da carteira individual (analista + estrategista)** — modernizar UI da carteira seguindo `briefing-carteira-analistas-ui.md` (hero card com gradiente, KPIs simplificados, gráfico com tooltip, tabela orientada a ação). Adicionar: mini-gráfico de crescimento por empresa na listagem (verde subindo/vermelho descendo/cinza regular), histórico de NPS do dono da carteira, bloco diferenciado por função (sugadores para analista / PPAs para estrategista). **REMOVER meta agregada da carteira** — meta é por empresa (modelo correto). Independente, pode rodar em paralelo. *(entregue 2026-06-29, checkbox corrigido no audit v12.0 2026-07-03)*
...
- [~] **Phase 50: Gamificação OAuth ML para Líder Performance + Estrategistas** *(v12.0 audit 2026-07-03: **WONT_DO** — descartada. Racional: v13.0 vai reorganizar sistema para multi-marketplace, gamificação OAuth ML isolada perde sentido nesse contexto. Se voltar a fazer sentido, replaneja como phase nova.)* — nova aba/rota incentivando conexão ML de empresas pendentes. Estrategista vê apenas empresas atribuídas; Líder vê todas. Ranking/badge/score por conexão concluída + status "Em conversa com cliente". Acelera migração Adman→ML. Independente; sinergia com Phases 41/42.
...
- [x] **Phase 52: Melhorias UI/UX + comportamento /sugadores** — 9 itens A1-A9 do briefing 2026-07-01 entregues em 4 waves (backend TDD + 2 UI + 1 correção). Nova rota `/sugadores/empresa/{id}` (EmpresaListagem) restaurou drilldown. Endpoints `mlbs-hint` (fix 422 ML-only) + `bulk-copy-mlbs` novos. UAT parcial aprovado após hotfix `Megaphone` não importado. Refinamentos identificados no UAT migram para Phase 54 + Phase 55.
- [~] **Phase 53: Inteligência do detector de sugadores** — Waves 1+2+hotfix Wave 4 em prod: cache 10min `listCampaigns` + `fetchItemStatus` (1h) + filtro MLB pausado (B1 CAMILLO); `sold_global>=10` remove `gasto_sem_venda` (B2 BARAOSHOP + B3 DINMAP); `campaign_name` persistido corretamente (hotfix Bug 2); `mlbsHint` fallback ML-only (hotfix Bug 1). 17/17 testes unit verdes. **STAND-BY** para observação orgânica ao longo de 3+ dias (cron `sugadores:analyze` das 12h BRT). Bugs diferidos: rate limit persistente, adgroup multi-MLB, Caso 2 config stale. Detalhes: `.planning/phases/53-inteligencia-detector-sugadores/53-STANDBY.md`.
- [x] **Phase 54: Refinamentos /sugadores UAT (correções + filtros)** — 5 itens A1-A3 + B1-B2 entregues em 4 waves. UAT 2026-07-02 exigiu 1 correção: filtro analista consulta CARGO (`user_setores → cargos.slug='analista'`), não pivot role. Memory `project_atribuicao_profissionais.md` documentava: analista = role 'consultor' na pivot. Phase 55 (Magic UI) agora destravada.
- [x] **Phase 55: Modernização visual /sugadores + Magic UI** — 3 waves entregues em 2026-07-03. C1 completo: Table shadcn + Checkbox Radix + actions dropdown MoreHorizontal + Customize Columns com localStorage v1. C2 reduzido pelo research (framer-motion custa 50-60kb): Blur Fade via `tailwindcss-animate` (0kb novos), Magic UI cortado, drag handles OUT. Bundle final +3.14 kB gzip no chunk EmpresaListagem (dentro do target +4kb). Zero regressão. UAT APROVADO direto.
...
- Shopee/Amazon nesta milestone: apenas stubs "em desenvolvimento" (sem integração real)
- Não mexer em rotas/dados existentes de ML antes de decidir o modelo multi-marketplace (evitar rework)
...
- [x] 58-02-PLAN.md — Componentes ShopeeShell.jsx + AmazonShell.jsx (mockup KPI cards + CTA para /dashboard/ecf) (DASH-03 UI)
...
- [x] 59-03-PLAN.md — Wave 3 (regressão + Publicação): `php artisan test` completo + comparação com baseline registrado no Plan 01 + Phase 57/58 baselines preservados (20/16) + reforço da seção "Publicação — CONFIRMED transversal" com evidência de suite verde. Cobre CROSS-02 (execução) + CROSS-03 (regressão)

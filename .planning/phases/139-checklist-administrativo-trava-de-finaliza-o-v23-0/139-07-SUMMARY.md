...
key-decisions:
  ...
  - "D-07 implementado: empresa isenta nunca tem exige_contrato=true, então o degrau 2→3 nunca dispara para ela — o salto 2→4 sai da ORDEM de checagem em proximoDestino(), sem ramo especial, e a tabela TRANSICOES_PERMITIDAS da Fase 137 já o previa"
  - "Guarda de legado (D-14 da Fase 138 / D-05 da Fase 137): etapa NULL devolve sem tocar em nada — zero linha em company_etapa_transicoes, provado por assertDatabaseMissing"
  - "Guarda de escopo: etapa fora de {1,2,3} devolve sem transicionar — a 4→5 é do FinalizarEntradaAdministrativaService e as posteriores são das Fases 141/142"
  ...
  - "ChecklistAdministrativoService NÃO foi alterado por este plano (git diff vazio) — quem dispara sincronizar() é a camada HTTP do plano 139-08, que injeta os três serviços como irmãos"
...
# Phase 139 Plan 07: ChecklistEtapaSincronizadorService — a régua da D-15 Summary

**O service que traduz o progresso do checklist administrativo nas transições de etapa 1→2, 2→3 e 2/3→4, sempre por `EtapaTransicaoService::transicionar()` — sem ele o FINALIZAR do plano 139-06 nasceria morto, porque a etapa 5 só é alcançável a partir da 4 e nenhum chamador de produção escrevia as etapas intermediárias.**
...
- `proximoDestino()` — helper privado que decide **um** degrau por iteração. A ordem de checagem é
  ...
  `TRANSICOES_PERMITIDAS` da Fase 137 já previa essa aresta.
...
- `tests/Feature/Phase139/ChecklistDirigeEtapaTest.php` — **10 casos, 33 assertions**, todos com
...
- Suíte da fase `tests/Unit/Phase139 + tests/Feature/Phase139` — **68 testes / 225 assertions,
  100% verde** (+10 testes / +33 assertions sobre 58/192 do fechamento do 139-06).
- Regressão obrigatória `tests/Unit/Phase137 + tests/Feature/Phase137 + tests/Feature/Phase138` —
  **123 testes / 444 assertions, 100% verde**, número por número **idêntico** ao
  `139-BASELINE-TESTES.md`. Este plano acrescenta o **terceiro** chamador de produção de
...
- `tests/Feature/Phase139/ChecklistDirigeEtapaTest.php` - 10 testes, D-15/D-07/D-14, 393 linhas

## Decisions Made

Nenhuma decisão nova de produto — as duas tasks implementam à letra a D-15 e a D-07, já travadas no
`139-CONTEXT.md`. Uma escolha **estrutural** foi feita dentro do espaço já previsto pela `<action>`:
...
(`ChecklistAdministrativoService` do 139-05, `FinalizarEntradaAdministrativaService` do 139-06,
`EtapaTransicaoService` da Fase 137).

## Next Phase Readiness

- `sincronizar()` está pronto para o plano **139-08** (camada HTTP), que precisa injetar os três
  ...
  `FinalizarTransicaoEtapaTest.php` (plano 139-06) prova que `finalizar()` isolado numa empresa
  fora da etapa 4 é recusado pela máquina de estados mesmo com o checklist 100% completo.
- **Proibição que o 139-08 herda:** não injetar o sincronizador dentro de
...
- Restam os planos **139-08** (endpoints HTTP), **139-09** e **139-10** para fechar a fase.
...
- FOUND: `tests/Feature/Phase139/ChecklistDirigeEtapaTest.php`

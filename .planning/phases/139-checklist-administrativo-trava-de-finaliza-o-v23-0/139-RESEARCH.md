# Phase 139: Checklist administrativo + trava de finalização (v23.0) - Research

**Researched:** 2026-09-09
**Domain:** Checklist de itens obrigatórios ancorado em `Company`, leitura de estado externo (Clicksign/ML OAuth), trava de transição de máquina de estados (`EtapaTransicaoService`, Fase 137)
...
- **D-09:** As permissões de módulo da Fase 138 bastam — `comercial.entrada` para os itens de Entrada e para o FINALIZAR, `admin.contratos` para os itens de Contrato. Nenhuma chave nova. As duas já são liberáveis por setor sem deploy (D-15 da Fase 138).
...
| ADMIN-03 | Link Adman, conexão ECF e Grant da consultoria gerados pelo próprio checklist, que marca o item ao gerar (D3 do REQUIREMENTS — nota: D-04/D-05 da Fase 139 corrigem a leitura original do D3) | Q1 (Grant = OAuth ML), Q4 (`OnboardingLinkService::paraEmpresa`), Q9 (link fixo Adman) |
...
| Transição de etapa (ADMIN-06) | API/Backend (`EtapaTransicaoService::transicionar()`, único ponto de escrita) | — | D-12 da Fase 137: nenhuma escrita direta de `companies.etapa` fora deste service |
...
| Trava de transição de etapa | `if` disperso em controller | `EtapaTransicaoService::transicionar()` | D-12 da Fase 137: único ponto de escrita, histórico automático em `company_etapa_transicoes` |
...
| Quick run command | `C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase139 tests/Feature/Phase139 --colors=never` |
| Full suite command (por wave) | `C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase139 tests/Feature/Phase139 tests/Feature/Phase138 tests/Feature/Phase131/ContratoAdminPermissaoTest.php tests/Unit/Phase137 tests/Feature/Phase137 --colors=never` |

> ⚠️ **`php artisan test` e `--testsuite=Feature` NÃO terminam neste ambiente** (travam em cascata de timeout de rede ~300s, herdado de `137-BASELINE-TESTES.md` e reconfirmado em `138-VALIDATION.md`). Rodar sempre por diretório/arquivo, nunca a suíte inteira.
>
...
Esta fase adiciona **novos chamadores** de `EtapaTransicaoService::transicionar()` (etapas 2/3/4/5) — comportamento novo sobre uma máquina de estados que já tem baseline própria. Capturar ANTES de qualquer código novo, em `139-BASELINE-TESTES.md`:
```
...
|--------|----------|-----------|-------------------|-------------|
| ADMIN-01 | Checklist aparece agrupado em Contrato/Entrada, com 9 itens (contrato) ou 6 (isento) | feature | `phpunit tests/Feature/Phase139/ChecklistContagemItensTest.php` | ❌ Wave 0 |
| ADMIN-02 | Itens 2/3 mudam sozinhos conforme `ContratoAssinatura` avança; item 1 é manual (exceção documentada, D-06) | feature | `phpunit tests/Feature/Phase139/ChecklistGrupoContratoAutoTest.php` | ❌ Wave 0 |
| ADMIN-03 | Itens 6 (link fixo)/7 (OAuth)/8 (conexão ECF) — 7 e 8 auto-marcam; 6 é manual (D-04) | feature | `phpunit tests/Feature/Phase139/ChecklistGrupoEntradaAutoTest.php` | ❌ Wave 0 |
| ADMIN-04 | Marcar manualmente grava `feito_por`/`feito_em`; desmarcar limpa os dois | feature | `phpunit tests/Feature/Phase139/ChecklistMarcacaoManualAutoriaTest.php` | ❌ Wave 0 |
| ADMIN-05 | FINALIZAR desabilitado com item pendente OU contrato não assinado; habilita no instante exato | feature | `phpunit tests/Feature/Phase139/FinalizarTravaTest.php` | ❌ Wave 0 |
| ADMIN-06 | FINALIZAR move companies.etapa → aguardando_distribuicao, mesmo company_id, `primaryMarketplace()` resolve destino | feature | `phpunit tests/Feature/Phase139/FinalizarTransicaoEtapaTest.php` | ❌ Wave 0 |
| D-08 (regressão) | Comercial/Entrada consegue abrir a mesma ficha sem 403 | feature | `phpunit tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php` — **depende da decisão do Pitfall 4** | ❌ Wave 0 |
...
### Sampling Rate
- **Per task commit:** comando rápido (`tests/Unit/Phase139 tests/Feature/Phase139`)
...
### Wave 0 Gaps
- [ ] `tests/Feature/Phase139/ChecklistContagemItensTest.php` — ADMIN-01
- [ ] `tests/Feature/Phase139/ChecklistGrupoContratoAutoTest.php` — ADMIN-02
- [ ] `tests/Feature/Phase139/ChecklistGrupoEntradaAutoTest.php` — ADMIN-03
- [ ] `tests/Feature/Phase139/ChecklistMarcacaoManualAutoriaTest.php` — ADMIN-04
- [ ] `tests/Feature/Phase139/FinalizarTravaTest.php` — ADMIN-05
- [ ] `tests/Feature/Phase139/FinalizarTransicaoEtapaTest.php` — ADMIN-06
- [ ] `tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php` — Pitfall 4 (depende de decisão do planejador)
...
- `.planning/phases/138-rea-comercial-conectada-etapa-v23-0/138-VERIFICATION.md` — citações de linha reusadas onde a Fase 138 já havia confirmado (ex.: ausência de corte por etapa em `ContratoAdminController`)
...
**Valid until:** 30 dias (código interno estável; revisitar se qualquer uma das Fases 137/138 sofrer hotfix antes do planejamento desta fase)

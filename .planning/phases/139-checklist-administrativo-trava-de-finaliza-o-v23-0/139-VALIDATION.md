...
# Phase 139 — Estratégia de Validação

> Contrato de validação por fase, para amostragem de feedback durante a execução.
> Derivado de `139-RESEARCH.md` §Validation Architecture, corrigido pelas decisões D-15..D-19.
...
|---|---|---|---|---|
| ADMIN-01 | Checklist agrupado em Contrato/Entrada; **9** itens com contrato, **6** para serviço isento (D-07); denominador do progresso acompanha | feature | `tests/Feature/Phase139/ChecklistContagemItensTest.php` | ❌ Wave 0 |
| ADMIN-02 | Itens 2/3 mudam sozinhos conforme `ContratoAssinatura` avança; item 1 ("revisado") é **manual** — exceção documentada (D-06, D-19) | feature | `tests/Feature/Phase139/ChecklistGrupoContratoAutoTest.php` | ❌ Wave 0 |
| ADMIN-03 | Itens 7 (OAuth ML) e 8 (conexão ECF) auto-marcam; item 6 (link fixo Adman) é **manual** (D-04) | feature | `tests/Feature/Phase139/ChecklistGrupoEntradaAutoTest.php` | ❌ Wave 0 |
| ADMIN-04 | Marcar manualmente grava `feito_por`/`feito_em`; **desmarcar limpa os dois juntos** (D-11); autoria sobrevive a usuário soft-deleted (`withTrashed()`) | feature | `tests/Feature/Phase139/ChecklistMarcacaoManualAutoriaTest.php` | ❌ Wave 0 |
| ADMIN-05 | FINALIZAR desabilitado com qualquer item pendente **ou** contrato não assinado; habilita no instante exato em que o último requisito cumpre — nunca antes | feature | `tests/Feature/Phase139/FinalizarTravaTest.php` | ❌ Wave 0 |
| ADMIN-06 | FINALIZAR move `companies.etapa` para `aguardando_distribuicao`, **mesmo `company_id`** (nenhum cadastro novo); `primaryMarketplace()` resolve o módulo de destino | feature | `tests/Feature/Phase139/FinalizarTransicaoEtapaTest.php` | ❌ Wave 0 |
| **D-15** | Progresso do checklist dispara as etapas: 1º item concluído → 2; envelope enviado → 3; tudo pronto → 4; FINALIZAR → 5. **Salto 2→4 para empresa isenta** (sem envelope não há etapa 3). Toda transição via `EtapaTransicaoService` | feature | `tests/Feature/Phase139/ChecklistDirigeEtapaTest.php` | ❌ Wave 0 |
| **D-16** | Item 3 fecha por `assinado_em`/`status === assinado` **OU** por `ContratoLiberacao` existente. Empresa liberada pela via `manual` (que não escreve `contrato_assinaturas`) **consegue** finalizar | feature | `tests/Feature/Phase139/ContratoAssinadoPorLiberacaoTest.php` | ❌ Wave 0 |
| **D-17** | Usuário só com `comercial.entrada` abre a ficha **sem 403** e **não vê** a seção Contrato; usuário com `admin.contratos` abre e vê | feature | `tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php` | ❌ Wave 0 |
| **D-18** | Com 2 envelopes ativos em estados diferentes, itens 2/3 ficam **pendentes** (manda o mais atrasado); só fecham quando TODOS atingem o estado | feature | `tests/Feature/Phase139/MultiplosEnvelopesTest.php` | ❌ Wave 0 |
...
| **D-10 (defesa)** | Chave de item desconhecida/órfã **não** entra no denominador do progresso — a ficha ainda fecha 100% | unit | `tests/Unit/Phase139/ChecklistProgressoTest.php` | ❌ Wave 0 |
...
- [ ] `tests/Feature/Phase139/ChecklistGrupoContratoAutoTest.php` — ADMIN-02 + D5
...
- [ ] `tests/Feature/Phase139/FinalizarTransicaoEtapaTest.php` — ADMIN-06 + D-12
- [ ] `tests/Feature/Phase139/ChecklistDirigeEtapaTest.php` — D-15
- [ ] `tests/Feature/Phase139/ContratoAssinadoPorLiberacaoTest.php` — D-16
- [ ] `tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php` — D-17
- [ ] `tests/Feature/Phase139/MultiplosEnvelopesTest.php` — D-18
- [ ] `tests/Unit/Phase139/ChecklistProgressoTest.php` — D-10
- [ ] `139-BASELINE-TESTES.md` — baseline 137/138 capturada

...
key-decisions:
  ...
  - "T-139-08-01 mitigada: sem admin.contratos o payload perde o grupo `contrato` do checklist e recebe contratos => [], pode_gerar_contrato => false, motivo_bloqueio => null. company/contratos_servico/faltantes continuam para os dois perfis — a listagem Entrada já os exibe, não há exposição nova"
...
# Phase 139 Plan 08: a camada HTTP que liga tudo Summary
...
- Gating por módulo (T-139-08-01): sem `admin.contratos`, o grupo `contrato` sai de
...
- Suíte da fase `tests/Unit/Phase139 + tests/Feature/Phase139` — **92 testes / 364 assertions,
  100% verde** (+24 testes / +139 assertions sobre 68/225 do fechamento do 139-07).
- Regressão `tests/Feature/Phase131 + tests/Feature/Phase138 + tests/Unit/Phase137 +
  tests/Feature/Phase137` — **245 testes / 910 assertions, 100% verde**. A parte 137/138 fecha em
  **123 testes / 444 assertions**, número por número **idêntica** ao `139-BASELINE-TESTES.md`; a
...
- `tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php` - 12 testes, 86 assertions
- `tests/Feature/Phase139/ChecklistEndpointsTest.php` - 12 testes, 53 assertions

## Decisions Made

Nenhuma decisão nova de produto — as três tasks implementam à letra D-17, D-08, D-09, D-04 e D-15,
já travadas no `139-CONTEXT.md`. Uma escolha **técnica** foi feita dentro do espaço que a `<action>`
...
publicada no plano 139-01.

## Next Phase Readiness

- O payload de `Admin/ContratoDetalhe` já entrega tudo o que o plano **139-09** (a UI) precisa:
...
- Os 4 endpoints estão prontos para os `router.post` do JSX. **Atenção para o 139-09:** o
...
- **Proibições que o 139-09 herda:** não reconstruir a régua do FINALIZAR no cliente (o botão lê
  `pode_finalizar`, nunca recalcula); e não hard-codar o link do Adman no JSX (D-04).
- Restam os planos **139-09** (UI) e **139-10** (fechamento da fase) para fechar a 139.
...
- FOUND: `tests/Feature/Phase139/ChecklistAcessoPorEntradaTest.php`
- FOUND: `tests/Feature/Phase139/ChecklistEndpointsTest.php`

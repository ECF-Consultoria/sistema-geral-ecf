...
# Dependency graph
requires:
  - phase: 137-maquina-de-estados-etapa-v23-0
...
provides:
  - "139-BASELINE-TESTES.md — baseline verde (123 testes, 444 assertions) capturada antes de qualquer código da fase 139"
...
affects: [139-02, 139-03, 139-04, 139-05, 139-06, 139-07, 139-08, 139-09, 139-10]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Chave de config configurável com default seguro (env('X', 'valor-fixo')) — mesmo padrão do bloco hubspot da Fase 138, aplicado ao Adman"
...
patterns-established:
  - "Baseline de testes por fase segue o mesmo formato/rigor do 138-BASELINE-TESTES.md: comando exato (duas grafias), resultado literal, suítes fora do gate com razão, advertência de ambiente"
...
# Phase 139 Plan 01: Baseline + config do Adman + registro de exceções nos requirements Summary
...
- `139-BASELINE-TESTES.md` registra baseline **100% verde** (123 testes, 444 assertions) das suítes `tests/Unit/Phase137` + `tests/Feature/Phase137` + `tests/Feature/Phase138`, capturada antes de qualquer código novo da fase — a referência que os planos 139-02 em diante usam para distinguir falha nova de falha herdada.
- `config/services.php` ganhou a terceira chave do array `adman` já existente (`register_url`), com `env('ADMAN_REGISTER_URL', 'https://app.ad-man.io/register?ref=588D0DD78C4F')` — o mesmo padrão de configurável-sem-deploy que a Fase 138 usou para o HubSpot.
...
- `tests/Unit/Phase139/AdmanRegisterUrlConfigTest.php` prova por teste automatizado que o default é exatamente o link fixo e que a URL não contém nenhum placeholder por empresa (`{company}`, `company_id`, `:id`).
...
1. **Task 1: Capturar a baseline de testes das Fases 137/138** - `35f9d193` (docs)
...
- `.planning/phases/139-checklist-administrativo-trava-de-finaliza-o-v23-0/139-BASELINE-TESTES.md` - baseline de testes 137/138, 100% verde, com as suítes de Polos explicitamente fora do gate
...
- A baseline de 137/138 (123/123 verde) está registrada e disponível para os planos 139-02 em diante compararem regressão.
...
- Nenhum bloqueio identificado para o plano 139-02.
...
- FOUND: `.planning/phases/139-checklist-administrativo-trava-de-finaliza-o-v23-0/139-BASELINE-TESTES.md`
- FOUND: `tests/Unit/Phase139/AdmanRegisterUrlConfigTest.php`

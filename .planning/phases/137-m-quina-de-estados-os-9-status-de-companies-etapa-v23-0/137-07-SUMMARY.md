...
provides:
  ...
  - "tests/Feature/Phase137/EtapaFiltroListagemTest.php — 9 testes provando os 6 cenários do contrato"
  - "Verificação humana real da tela, aprovada pelo usuário"
  - "Fase 137 fecha as 6 Success Criteria — ETAPA-05 era o último requirement pendente"
affects: [138, 142]
...
key-decisions:
  ...
  - "Item 7 (Task 3 <action>): full php artisan test substituído pelo subset seguro (Unit/Phase137 + Feature/Phase137 + as duas suítes baseline de /companies) — a suíte Feature completa não termina neste worktree (trava em timeout de rede de 300s, medido por 137-01 ANTES de qualquer código desta fase existir). Ver seção 'Dívida explícita' abaixo"
...
# Phase 137 Plan 07: Filtros server-side de etapa e pendência em /companies Summary

**Dois filtros independentes e combináveis (`?etapa=` com allow-list + sentinela `sem_etapa` de primeira classe, e `?com_pendencia=` via `comPendenciaAberta()`) na aba Empresas de `/companies`, verificados por um humano na tela real — fecha a Fase 137 (todas as 6 Success Criteria e os 6 requirements ETAPA-01..06).**
...
- `tests/Feature/Phase137/EtapaFiltroListagemTest.php` — 9 testes cobrindo etapa concreta, `sem_etapa`, valor inválido (fallback), pendência isolada, combinação etapa+pendência, visão padrão sem filtro (Success Criteria nº 1 da fase) e a prop `filters`
...
- **A Fase 137 fecha inteira com este plano** — ETAPA-05 era o único requirement dos 6 (`ETAPA-01`..`ETAPA-06`) ainda pendente; as 6 Success Criteria do ROADMAP para a Fase 137 estão todas satisfeitas
...
Comando:
```
C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase137 tests/Feature/Phase137 tests/Feature/Phase37CompaniesPerformanceFilterTest.php tests/Feature/V16/CompanyControllerResponsavelPerformanceTest.php --colors=never
```
...
- **`php artisan test` / `--testsuite=Feature` não termina neste worktree.** Trava numa cascata de timeout de rede de 300s (chamada Guzzle não mockada, alvo não confirmado — candidatos prováveis: Adman, OAuth ML/Google, ou SFTP de grants). Medido primeiro por `137-01` antes de qualquer código desta fase existir; re-confirmado nesta sessão isolando 4 suítes candidatas (todas verdes, 28/28). **O teste exato que dispara o timeout não foi identificado.** Próxima sessão que precisar de `php artisan test` verde: não rodar `--testsuite=Feature` inteiro esperando que termine — isolar por diretório, como `137-BASELINE-TESTES.md` já recomenda.
...
- **Issue:** Expor a chave `pendencia_aberta` no payload de `/companies` faria o gate estático de `tests/Feature/Phase137/EtapaPendenciaParaleloTest.php` (plano 137-04, D-19) acusar uma leitura direta do flag fora de `Company.php`, mesmo sendo, de fato, uma leitura através do método `pendenciaAberta()`.
...
- **Verification:** `EtapaFiltroListagemTest.php` (9/9) e `EtapaPendenciaParaleloTest.php` (suíte do 137-04, incluída em `tests/Feature/Phase137`) ambos verdes no subset seguro.
...
- **Fase 137 está completa** — as 6 Success Criteria do ROADMAP e os 6 requirements (`ETAPA-01`..`ETAPA-06`) estão fechados. Não há plano 137-08.
- Fase 138 (Área Comercial conectada à etapa) depende da Fase 137 e pode começar — o filtro por etapa entregue aqui é a listagem de referência (`/companies`), a listagem do Comercial (COMERC-02) é escopo próprio da 138, não reuso direto deste componente.
- Fase 142 (Onboarding plugado na máquina de estados) é quem troca o critério `em_operacao` (hoje derivado) pela leitura direta de `etapa` — este plano deliberadamente NÃO tocou esse cálculo, só acrescentou filtro ao lado dele.
- Pendências de produção que sobrevivem à Fase 137 inteira, sem relação com este plano especificamente: o backfill em produção e a conferência da aba Empresas antes/depois lá continuam Manual-Only (`137-VALIDATION.md`), e exigem autorização explícita de deploy antes de acontecer — nenhum deploy saiu desta fase.
...
- FOUND: .planning/phases/137-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/137-07-SUMMARY.md

# Phase 137: Máquina de estados — os 9 status de `companies.etapa` (v23.0) - Research
...
(Fase 108) é praticamente o desenho que a Fase 137 precisa para transição de etapa, só trocando
os nomes dos campos.

**Recomendação principal:** trate a Fase 137 como puramente aditiva e sem chamador de produção
...
ter dado real) do risco de qualquer tela nova, e é consistente com o próprio ROADMAP: a Fase 137
**preserva** a tela `/companies`, a Fase 142 é que troca o critério.
...
- **D-01:** Os 9 valores são constantes `public const ETAPA_*` no model `Company`, em
  ...
  já expõe — a Fase 142 troca a fonte sem trocar o nome.
...
- **D-09:** A etapa é uma só, na `companies`. Regra travada: manda o onboarding mais atrasado.
  ...
  concluírem.
- **D-10:** A Fase 137 declara essa regra e desenha o serviço para comportá-la, mas não a
  implementa — quem liga onboarding à etapa é a Fase 142.
...
- **D-13:** O serviço recebe o `User` que agiu e o registra. A Fase 137 não constrói a matriz de
  permissão por papel.
...
- Painel de SLA agregado — HIST-03 (Fase 143) entrega o dado por empresa; o painel é produto
  separado.
...
| Registrar histórico da transição (D-16) | Database / Storage | API / Backend | Nasce como efeito colateral de `transicionar()`, mas a leitura (Fase 143) é backend puro sobre a tabela/activity_log |
...
| Botão/UI de transição | Browser / Client | — | Fora do escopo declarado desta fase (ADMIN-05/DISTRIB-03/ONBRD-01 são das Fases 139/141/142); a 137 só garante que o serviço existe e está pronto para ser chamado |
...
│ motor de onboarding, tela da Coordenação — nascem nas Fases 138-142) │
└───────────────────────────────┬───────────────────────────────────────┘
...
│ Fase 143)           │   │ (ETAPA-05, server-side, D-21) │
...
```
Para a Fase 137, `podeTransicionar()` faz o papel de `avaliar()` e `transicionar()` faz o papel
...
**Aplicação à Fase 137:** o `EtapaTransicaoService` pode (e talvez deva) nascer sem nenhum
controller/job de produção chamando `transicionar()` ainda — só testes. Quem liga o serviço a um
gatilho real de produção é a Fase 138 (webhook → etapa 1), 139 (FINALIZAR → etapa 5), 141
(distribuição → etapa 6) e 142 (onboarding → 7/8/9). Isso é consistente com o próprio Success
Criteria 2 da Fase 137 ("não existe outro ponto do código que grave `companies.etapa`") — a
forma mais segura de garantir isso é a Fase 137 não plugar nenhum chamador de verdade, e sim
...
Para a Fase 137, o equivalente seria algo como
...
**Contras:** "quanto tempo em cada etapa" (HIST-03, Fase 143) exige olhar `properties->de`/
...
fechado (9 valores + retrocesso). Se HIST-03 (Fase 143, "quanto tempo em cada etapa, sem abrir o
banco") é a motivação central de D-16, colunas tipadas evitam que a Fase 143 tenha que reescrever
...
|--------|----------|-----------|-------------------|-------------|
| ETAPA-01 | `Company::ETAPAS` tem os 9 valores, na ordem, e a migration cria `etapa` nullable sem default | unit | `phpunit tests/Unit/Phase137/CompanyEtapaConstantesTest.php` | ❌ Wave 0 |
| ETAPA-02 | Backfill: balde 1 (analista OU estrategista) → etapa 9; balde 2 → NULL; tela `/companies` mostra o mesmo conjunto de antes | feature | `phpunit tests/Feature/Phase137/EtapaBackfillTest.php` (molde: `Phase37CompaniesPerformanceFilterTest.php`, que já prova o payload de `/companies` linha a linha) | ❌ Wave 0 |
| ETAPA-03 | `PUT /companies/{company}` com `etapa` no payload NÃO muda `companies.etapa`; nenhum outro `Company::etapa =`/`update(['etapa' => ...])` existe fora do serviço | feature + grep estático | `phpunit tests/Feature/Phase137/EtapaPontoUnicoTest.php` + `grep -rn "'etapa'\s*=>" app/ --include=*.php` limitado ao arquivo do serviço | ❌ Wave 0 |
| ETAPA-04 | Marcar/desmarcar pendência não muda `etapa`; accessor único de leitura devolve o valor certo em todos os casos | unit + feature | `phpunit tests/Feature/Phase137/EtapaPendenciaParaleloTest.php` | ❌ Wave 0 |
| ETAPA-05 | `?etapa=X` e `?com_pendencia=1` filtram server-side, combináveis, com `sem_etapa` como opção de primeira classe (D-22) | feature | `phpunit tests/Feature/Phase137/EtapaFiltroListagemTest.php` (molde: filtro `cust_id_status` já testado em `Phase37CompaniesPerformanceFilterTest.php::test_filtro_cust_id_status_invalido_continua_funcional`) | ❌ Wave 0 |
| ETAPA-06 | `podeTransicionar()` recusa transição inválida com `requisito_faltante` nomeado (nunca mensagem genérica); tabela de transições da D-14 cobre o salto conhecido 2→4 | unit | `phpunit tests/Unit/Phase137/EtapaTransicaoServiceTest.php` | ❌ Wave 0 |

### Sampling Rate

- **Por commit de task:** rodar as suítes específicas de `tests/{Unit,Feature}/Phase137/` (rápido,
  segundos).
- **Por merge de wave:** rodar a Quick run command acima (as duas suítes de `/companies` que já
  existem) + `tests/Phase137/*` completo.
...
- [ ] `tests/Unit/Phase137/CompanyEtapaConstantesTest.php` — cobre ETAPA-01
- [ ] `tests/Feature/Phase137/EtapaBackfillTest.php` — cobre ETAPA-02 (D-04/D-05/D-08)
- [ ] `tests/Feature/Phase137/EtapaPontoUnicoTest.php` — cobre ETAPA-03 (incluindo o caso do
  Pitfall 1: `PUT /companies/{company}` com `etapa` no payload)
- [ ] `tests/Feature/Phase137/EtapaPendenciaParaleloTest.php` — cobre ETAPA-04
- [ ] `tests/Feature/Phase137/EtapaFiltroListagemTest.php` — cobre ETAPA-05
- [ ] `tests/Unit/Phase137/EtapaTransicaoServiceTest.php` — cobre ETAPA-06
...
### Superfícies de risco identificadas (sem construir a matriz de permissão — deferida às Fases 138/139/141)
...
2. **Endpoint de transição, quando nascer (fora do escopo da 137, mas o serviço precisa estar
   ...
   (Fases 138+):** sempre passar `$request->user()`/`auth()->user()`, nunca aceitar `user_id` do
...
- `.planning/CONTEXT.md` da Fase 137 — decisões do usuário (D-01..D-23), tratadas como travadas, não re-verificadas

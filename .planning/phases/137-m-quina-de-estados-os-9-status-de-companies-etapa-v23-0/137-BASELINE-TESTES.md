...
# Baseline de testes — Fase 137 (antes de qualquer migration)

Coletada **antes** de qualquer migration desta fase existir (137-02 e 137-05 são as que
alteram `companies` com dado de produção). Exigência do `CLAUDE.md` § "GSD obrigatório". Este
plano (`137-01`) **não** modifica nenhum arquivo em `app/`, `database/` ou `resources/` — todo
...
**Consequência para os planos seguintes (137-02..07):** como o caminho foi `npm-build`, **não**
...
que a pesquisa mediu (`137-VALIDATION.md` linha 26: "medido: 24 testes / 44 assertions").

## Quick run command (suíte dedicada da fase)

`tests/Unit/Phase137` e `tests/Feature/Phase137` **ainda não existem** — nascem nos planos
137-02 em diante (tdd_mode desligado nesta fase; cada suíte nasce junto da implementação que
prova, ver `137-VALIDATION.md` § "Wave 0 Requirements"). Nada a rodar aqui.
...
vermelho). Todas as 12 ocorrências são pré-existentes, não relacionadas à Fase 137:
...
**Isto não é regressão da Fase 137** — nenhum arquivo de `app/`, `database/` ou `resources/`
foi tocado por este plano (137-01). É estado pré-existente do worktree: aparentemente nenhuma
sessão anterior tinha rodado a suíte Feature completa neste ambiente sem rede antes de hoje.

**Para as próximas sessões desta fase (137-02..137-07) e para `/gsd:verify-work`:** não rodar
...
- `Quick run command` (`tests/Unit/Phase137 tests/Feature/Phase137`) a cada task — é o que o
  `137-VALIDATION.md` já prescreve como sampling rate.
...
Nenhuma falha listada acima é regressão da Fase 137 — este documento é a baseline "antes".
Comparação de regressão nos próximos planos (137-02..07) é sempre contra estes números, nunca
contra zero:

# Phase 137: Máquina de estados — os 9 status de `companies.etapa` (v23.0) - Discussion Log
...
136 — `init.phase-op` devolveu `phase_found: false`. A Fase 137 vive no worktree
...
| Dobrar na Fase 137 | Tratar as 2 rotas como parte do ponto único de escrita | |
...
- **Deduzir etapa 2..8 no backfill a partir de `ContratoAssinatura`/`Onboarding.status`** —
  ...
  duração fictícia no painel de SLA da Fase 143.
...
- Matriz de permissão por papel nas transições — Fases 138/139/141

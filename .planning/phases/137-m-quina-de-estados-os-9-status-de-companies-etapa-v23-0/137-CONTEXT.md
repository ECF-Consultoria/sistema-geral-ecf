# Phase 137: Máquina de estados — os 9 status de `companies.etapa` (v23.0) - Context
...
(Fase 138), checklist administrativo e botão FINALIZAR (Fase 139), mensagem de boas-vindas
(Fase 140), tela de distribuição da Coordenação (Fase 141), plugar o onboarding nas etapas
7/8/9 (Fase 142), timeline e SLA (Fase 143).

**Não muda nesta fase:** o critério da aba "Empresas" de `/companies` continua sendo o
cálculo derivado atual. A troca do derivado por `etapa` é da Fase 142 — a Fase 137 **acrescenta**
...
- **D-01:** Os 9 valores são constantes `public const ETAPA_*` no model `Company`, em
  ...
  `CompanyController` já expõe — a Fase 142 troca a fonte sem trocar o nome.
...
- **D-05:** **O backfill NUNCA deduz etapa intermediária (2..8) a partir de estado
  ...
  - O Success Criteria nº 1 é "a tela não perde nenhuma empresa que mostra hoje". Carimbar
    ...
    quando a Fase 142 passar a ler `etapa`.
  ...
  - A Fase 143 mede SLA por tempo em etapa. Etapa deduzida vira duração fictícia no painel
    de gargalo.
...
- **D-10:** A Fase 137 **declara** essa regra e desenha o serviço para comportá-la, mas
  **não a implementa** — quem liga onboarding à etapa é a Fase 142. O recorte exato de
...
- **D-11:** O serviço **não deriva etapa de estado externo** — ele transiciona por **chamada
  ...
  etapa fosse derivada, ela não estaria realmente armazenada, e a Fase 143 (HIST-03, "quanto
...
- **D-13:** O serviço recebe o `User` que agiu e o registra. A Fase 137 **não** constrói a
  matriz de permissão por papel (Comercial × Administrativo × Coordenação) — isso é das
  Fases 138/139/141 —, mas a assinatura já carrega o ator porque DISTRIB-03 e HIST-01/02
...
- **D-16:** Toda transição emite evento/log com ator, origem, destino e instante — é o
  insumo que a Fase 143 vai consumir. A Fase 137 **emite e registra**; a **timeline e o
...
- **D-17:** Pendência é **campo declarado à mão**, não derivação. Quatro motivos:
  ...
  - ADMIN-04 (Fase 139) já exige "quem marcou e quando" para os itens manuais — mesma forma.
...
- **D-20:** A listagem que ganha o filtro nesta fase é **`/companies`**
  ...
  Comercial é tratada pela Fase 138 (COMERC-02).
...
- `.planning/ROADMAP.md` — bloco "Milestone v23.0" (linha ~2076) e "Phase 137" (~2090).
...
- **`app/Http/Controllers/ComercialController.php:594`** e
  **`app/Http/Controllers/Api/HubspotWebhookController.php`** — os **dois** pontos que
  criam `Company` hoje. São os dois lugares que a Fase 138 vai fazer nascer na etapa 1;
...
- **`app/Models/Onboarding.php:54-56`** — `STATUS_RASCUNHO`/`ANDAMENTO`/`CONCLUIDO`: o
  estado real por trás das etapas 7 e 8, consumido pela Fase 142.
- **`app/Http/Controllers/OnboardingController.php:379`** — `responsavel_analista_id` /
  ...
  estrategista", diferente do pivô `company_users`. A Fase 141 vai ter de escolher qual é
...
- **Painel de SLA agregado** — já reconhecido em Future Requirements do REQUIREMENTS-v23.
  HIST-03 (Fase 143) entrega o dado por empresa; o painel é produto separado.
...
- **`260818-portas-extras-criam-mlbempresa-fora-do-router.md`** — 2 rotas HTTP criam
  ...
  princípio de "cadastro único" do PDF, mas o usuário decidiu **deixar fora** da Fase 137:

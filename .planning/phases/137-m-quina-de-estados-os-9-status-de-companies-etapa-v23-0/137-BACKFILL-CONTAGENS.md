# Fase 137 Plano 05 — Contagens do backfill (D-08, reconsulta ao banco)
...
(deliberado, para não inventar duração fictícia no painel de gargalo da Fase 143).
...
`137-RESEARCH.md` § "Contagem real" (medida em pesquisa: balde 1 = 1, balde 2 = 179, total 180).
...
volume ou a proporção esperados em produção. `137-RESEARCH.md` já registrava essa ressalva antes
...
2. Capturar a lista de empresas (ids + nomes) que a aba "Empresas" de `/companies` mostra em
   produção **antes** do backfill (Manual-Only Verification do `137-VALIDATION.md`).
...
transição não tem chamador de produção — ver `137-03-SUMMARY.md`), e `carimbarBackfill()` não

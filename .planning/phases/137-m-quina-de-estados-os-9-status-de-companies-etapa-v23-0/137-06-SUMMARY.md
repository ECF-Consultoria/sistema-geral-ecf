...
affects: [137-07, 138, 139, 141, 142]
...
# Phase 137 Plan 06: Ponto único de escrita de companies.etapa — regressão permanente Summary
...
- `PUT /companies/{company}` provado, por request HTTP real (não mock), incapaz de mover `etapa` ou marcar `pendencia_*` mesmo enviando o payload completo aceito hoje pela validação — T-137-01 e T-137-14 fechados
...
- Success Criteria nº 2 da Fase 137 provado automatizado e permanente
...
- `tests/Feature/Phase137/EtapaPontoUnicoTest.php` - 4 testes: 3 comportamentais (PUT com etapa válida, etapa inválida, pendência) + 1 de varredura estática escopada por corpo de função
...
- **Files modified:** tests/Feature/Phase137/EtapaPontoUnicoTest.php
...
- Success Criteria nº 2 da Fase 137 ("nenhum outro ponto do código grava companies.etapa além do serviço de transição") está provado e permanente — `tests/Feature/Phase137/EtapaPontoUnicoTest.php` roda a cada `Quick run command` (`tests/Unit/Phase137 tests/Feature/Phase137`, 37/37 verde) e a cada `Baseline command` (24/24 verde, sem regressão)
- Plano 137-07 (filtro server-side de etapa em `/companies`) pode tocar `CompanyController.php` livremente na LISTAGEM (`index()`) sem risco de esbarrar neste gate — o gate só reage a escrita, nunca a leitura (`where('etapa', ...)` já é tolerado, é o próprio padrão que o filtro do 137-07 vai usar)
- Nenhum bloqueio para a Fase 138 (webhook → etapa 1) nem para as fases seguintes que vão chamar `EtapaTransicaoService::transicionar()` pela primeira vez em produção — o gate deste plano garante que elas serão, de fato, o único caminho de escrita
...
- FOUND: tests/Feature/Phase137/EtapaPontoUnicoTest.php
- FOUND: .planning/phases/137-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/137-06-SUMMARY.md

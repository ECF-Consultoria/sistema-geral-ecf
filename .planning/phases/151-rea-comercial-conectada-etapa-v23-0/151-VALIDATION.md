...
# Phase 151 — Estratégia de Validação

> Contrato de validação por fase, para amostragem de feedback durante a execução.
> Derivado de `151-RESEARCH.md` § "Validation Architecture".
...
| **Comando rápido** | `C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase151 tests/Feature/Phase151 --colors=never` |
| **Comando de suíte (por wave)** | `C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase151 tests/Feature/Phase151 tests/Feature/Phase131/ContratoAdminPermissaoTest.php tests/Unit/Phase150 tests/Feature/Phase150 --colors=never` |
...
> `150-BASELINE-TESTES.md`. **Rodar sempre por diretório/arquivo**, nunca a suíte Feature inteira.
>
...
alterar coluna existente) — mais branda que a da Fase 150. Ainda assim `companies` tem ~500 registros
em produção, e a regra do `CLAUDE.md` ("migration que altere tabela existente com dado em produção")
se aplica.

**Capturar ANTES da migration**, em `151-BASELINE-TESTES.md`:
...
C:\xampp\php\php.exe vendor/bin/phpunit \
  ...
  tests/Unit/Phase150 tests/Feature/Phase150 --colors=never
```
...
- **A cada commit de task:** comando rápido (`tests/Unit/Phase151 tests/Feature/Phase151`)
...
|--------|------------|------|-------------------|-------------|
| COMERC-01 | Webhook cria empresa → nasce em `aguardando_administrativo` | feature | `phpunit tests/Feature/Phase151/ComercEtapaNascimentoWebhookTest.php` | ❌ Wave 0 |
| COMERC-01 | Cadastro manual (`ComercialController::store`) nasce na mesma etapa, pelo mesmo serviço | feature | `phpunit tests/Feature/Phase151/ComercEtapaNascimentoCadastroManualTest.php` | ❌ Wave 0 |
...
| COMERC-02 | Listagem **Contrato** mostra os 8 campos do §2, incluindo owner e `data_venda` | feature | `phpunit tests/Feature/Phase151/ComercListagemContratoCamposTest.php` | ❌ Wave 0 |
| COMERC-02 | Listagem **Entrada** mostra os 8 campos; a mesma empresa pode aparecer nas duas listas | feature | `phpunit tests/Feature/Phase151/ComercListagemEntradaTest.php` | ❌ Wave 0 |
...
| COMERC-03 | Empresa em `administrativo_concluido` (4) continua visível; em `aguardando_distribuicao` (5) some (D-07) | feature | `phpunit tests/Feature/Phase151/ComercVisibilidadeAteEtapa5Test.php` | ❌ Wave 0 |
...
| D-16 (regressão) | `admin.empresas` continua acessível por URL direta mesmo fora do menu | feature | teste leve novo em `tests/Feature/Phase151/` | ⚠️ conferir cobertura atual |
| D-17 | Ator de sistema não configurado/não encontrado → log e **não** transiciona (`etapa` NULL), nunca `TypeError`/500 | feature | `phpunit tests/Feature/Phase151/ComercAtorSistemaAusenteTest.php` | ❌ Wave 0 |
...
- [ ] `tests/Feature/Phase151/ComercEtapaNascimentoWebhookTest.php` — COMERC-01 (webhook)
- [ ] `tests/Feature/Phase151/ComercEtapaNascimentoCadastroManualTest.php` — COMERC-01 (manual)
- [ ] `tests/Feature/Phase151/ComercListagemContratoCamposTest.php` — COMERC-02 (Contrato)
- [ ] `tests/Feature/Phase151/ComercListagemEntradaTest.php` — COMERC-02 (Entrada)
- [ ] `tests/Feature/Phase151/ComercVisibilidadeAteEtapa5Test.php` — COMERC-03
- [ ] `tests/Feature/Phase151/ComercAtorSistemaAusenteTest.php` — D-17 (fallback seguro)

...
deferred:
  - truth: "Nome interno real da property de owner (`hubspot_owner_id`) medido contra a conta HubSpot real"
    addressed_in: "Deploy da própria Fase 151 (roteiro na 138-CONTA-SISTEMA-HUBSPOT.md)"
  ...
  - truth: "Escopo OAuth crm.objects.owners.read confirmado no Private App do HubSpot"
    addressed_in: "Deploy da própria Fase 151"
...
# Fase 151: Área Comercial conectada à etapa (v23.0) — Relatório de Verificação
...
- `app/Http/Controllers/Api/HubspotWebhookController.php:423` — `reprocessarEvento()` chama o
  mesmo `nascerNaEtapa1($company)` — a lacuna relatada pelo plano 151-07 (bypass do gate
...
- `git diff fb7ddfd0..HEAD -- app/Services/FluxoEntrada/EtapaTransicaoService.php` → **vazio**: o
  serviço da Fase 150 não foi tocado nem reaberto por esta fase.
- Testado por `tests/Feature/Phase151/ComercEtapaNascimentoWebhookTest.php` (6 cenários,
  incluindo ator ausente e ator inexistente) e
  `tests/Feature/Phase151/ComercEtapaNascimentoCadastroManualTest.php`. Reexecutei ambos nesta
...
- Testado por `tests/Feature/Phase151/ComercListagemContratoCamposTest.php` e
  `tests/Feature/Phase151/ComercListagemEntradaTest.php` — reexecutados, verdes.
...
- `tests/Feature/Phase151/ComercVisibilidadeAteEtapa5Test.php` — 7 testes cobrindo os dois
...
```
C:\xampp\php\php.exe vendor/bin/phpunit tests/Unit/Phase151 tests/Feature/Phase151 \
  tests/Feature/Phase131 tests/Unit/Phase150 tests/Feature/Phase150 --colors=never
```
...
(`151-BASELINE-TESTES.md`), sem regressão nas suítes que exercitam a `companies.etapa` da Fase 150.
...
6. **Fixtures locais `[TESTE 151-09]` (ids 408-417)** — deixadas de propósito no banco de dev
...
|---|---|---|---|
| COMERC-01 | 151-07 | ✓ SATISFEITO | ver seção acima |
| COMERC-02 | 151-03, 151-04, 151-05, 151-06 | ✓ SATISFEITO | ver seção acima |
| COMERC-03 | 151-05 | ✓ SATISFEITO | ver seção acima |

Nenhum requirement órfão: `REQUIREMENTS-v23.md` mapeia exatamente COMERC-01/02/03 para a Fase 151,
...
`TODO`/`HACK`/`PLACEHOLDER` funcional (as únicas ocorrências de "checklist"/"Fase 152" em
...
documentada na tela, como o `151-CONTEXT.md` (D-06) prescreve: "nada finge estar pronto"). Nenhum
...
**A Fase 151 atingiu o objetivo declarado.** As três verdades que o ROADMAP e o REQUIREMENTS-v23
...
suíte de regressão da Fase 150/131 continua verde sem alteração de contagem. A prioridade de
...
local desta fase e não bloqueiam o avanço para a Fase 152. Nenhum deploy foi feito, conforme

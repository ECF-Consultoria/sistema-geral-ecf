...
affects: [151-04, 151-05, 151-06, 151-09]
...
key-files:
  created:
    ...
    - tests/Feature/Phase151/CompaniesColunasOwnerVendaTest.php
    - tests/Unit/Phase151/HubspotOwnerResolverTest.php
  modified:
...
key-decisions:
  ...
  - "Comentário do config/services.php explicita que o default 'hubspot_owner_id' foi ASSUMIDO, NÃO MEDIDO (401 por falta de HUBSPOT_ACCESS_TOKEN local), com autorização explícita do usuário em 2026-09-02 — medição real pendente do plano 151-09 contra a VPS"
...
# Phase 151 Plan 03: Colunas de owner/data da venda + resolução do responsável comercial Summary
...
- `tests/Feature/Phase151/CompaniesColunasOwnerVendaTest.php` - 5 testes: existência das colunas, nascimento null, cast de data, ausência de obrigatoriedade, atribuição por Eloquent
...
- `tests/Unit/Phase151/HubspotOwnerResolverTest.php` - 7 testes: nome composto, fallback e-mail, id nulo (2 variantes), 403, 404, cache (`assertSentCount(1)`)

## Decisions Made
- Nenhuma decisão nova além das já travadas em `151-CONTEXT.md` (D-08, D-09) e da resolução herdada de `151-02` (nome da property assumido com autorização explícita, medição real adiada para a VPS via plano 151-09).
...
None - nenhuma configuração de serviço externo é exigida por este plano. A pendência de medição real da property `hubspot_owner_id` e do escopo `crm.objects.owners.read` contra a conta HubSpot de produção já está registrada como obrigação do plano **151-09** (ver `138-HUBSPOT-MEDICOES.md`), não deste plano.

## Next Phase Readiness

A capacidade está pronta para os planos 151-04 (persistência — gravar `hubspot_owner_id`/`hubspot_owner_nome`/`data_venda` a partir do webhook/reprocessamento) e 151-05/151-06 (exibição nas listagens Contrato e Entrada). Nenhum bloqueio conhecido. A pendência de medição real na VPS (nome da property + escopo OAuth) permanece rastreada em `138-HUBSPOT-MEDICOES.md` e é responsabilidade do plano 151-09, não impede o trabalho dos planos 151-04/05/06 porque o `env()` override cobre a correção sem mudança de código.

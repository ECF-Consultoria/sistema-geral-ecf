# 151-02 — Medições contra a conta HubSpot real da ECF
...
Isso corresponde à opção **(b)** oferecida no `how-to-verify` do plano (`151-02-PLAN.md`):
...
plano 151-03) nasce sempre vazia — sem erro visível em lugar nenhum. Esse é exatamente o modo de
...
do plano **151-09** (checkpoint final da fase).

**Nenhum arquivo de código foi tocado por esta resolução** — `hubspot_owner_id` é o nome que o
plano 151-03 vai fixar como default de `env('HUBSPOT_PROP_DEAL_OWNER_ID', 'hubspot_owner_id')`
...
Este item também é carregado como pendência obrigatória do plano **151-09**, para ser conferido
depois do primeiro deploy — junto com a medição da property acima, ambas contra a conta HubSpot
real via VPS.

## Pendências obrigatórias para a VPS (item do plano 151-09)
...
em aberto com autorização explícita do usuário. O plano 151-09 (checkpoint final da fase) DEVE
...
1. **Medir o nome interno real da property de owner na VPS.**
   ...
   - Se o `name` devolvido for **diferente** de `hubspot_owner_id`: setar
     `HUBSPOT_PROP_DEAL_OWNER_ID=<nome_real>` no `.env` da VPS. O config do plano 151-03 lê o
...
## Confirmação no checkpoint do plano 151-09 (2026-09-09)

No checkpoint da Task 2 do plano 151-09, questionado se o escopo `crm.objects.owners.read`

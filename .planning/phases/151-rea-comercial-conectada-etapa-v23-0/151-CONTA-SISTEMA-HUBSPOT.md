...
## Confirmação por reconsulta ao banco (plano 151-09, Task 1)
...
## Suíte da fase × baseline pré-migration (`151-BASELINE-TESTES.md`) — medido 2026-09-09

| Suíte | Baseline (`151-01`) | Medido agora (`151-09`) | Veredito |
|---|---|---|---|
| `tests/Unit/Phase151 tests/Feature/Phase151` | não existia ainda | **72 tests, 291 assertions — OK** | verde, sem baseline anterior para comparar (suíte nova da própria fase) |
| `tests/Feature/Phase131 tests/Unit/Phase150 tests/Feature/Phase150 tests/Feature/Phase34HubspotWebhookTest.php` | — (comando de wave não media este conjunto exato) | **186 tests, 675 assertions — OK** | verde, nenhuma regressão |
...
Nota sobre exit code: `151-BASELINE-TESTES.md` registra exit code 0 para os comandos de Polos com
...
A não-logabilidade fica provada, em vez disso, por `tests/Feature/Phase151/ComercAtorSistemaTest.php`
— reexecutado nesta resolução (`/c/xampp/php/php.exe vendor/bin/phpunit
tests/Feature/Phase151/ComercAtorSistemaTest.php --colors=never` → **OK, 5 tests, 26 assertions**):
...
- **Tela Entrada** (`/comercial/entrada`): 11 colunas — Empresa, CNPJ, Serviços, Setor, Origem,
  ...
  8 empresas `[TESTE 151-09]` de etapas 1 a 4. Etapa renderizada legível ("Administrativo
  Concluído", não a chave crua). Nenhum item de checklist nem botão FINALIZAR. O texto de topo
  declara que os itens do checklist chegam na Fase 152.
...
- As empresas `[TESTE 151-09]` NÃO aparecem na listagem Contrato porque têm **zero serviços
...
- **179 das 190 empresas locais têm `etapa` NULL** e a tela Contrato as mostra como "Sem etapa
  ...
  legadas na etapa 1 — falso, e inundaria a listagem da Fase 151.
...
- A conferência da **barra lateral** (grupo Comercial com Contrato e Entrada; grupo
  ...
  existe em `tests/Feature/Phase151/ComercNavegacaoReorganizadaTest.php` e
...
`/c/xampp/php/php.exe vendor/bin/phpunit tests/Feature/Phase151 tests/Feature/Phase131 --colors=never`
→ **OK, 187 tests, 746 assertions**, exit code 0.

**Fixtures locais usadas na verificação (não removidas — não é escopo deste plano):** ids
**408-417**, 10 empresas `[TESTE 151-09]`, duas por etapa de 1 a 5. Empresa `asdadassdsad`

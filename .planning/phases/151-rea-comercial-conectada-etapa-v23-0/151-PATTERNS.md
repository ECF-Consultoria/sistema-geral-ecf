# Phase 151: Área Comercial conectada à etapa (v23.0) - Mapa de Padrões
...
| `tests/Feature/Phase151/*.php` (6 arquivos) | test | request-response / event-driven | `tests/Feature/Phase131/ContratoAdminPermissaoTest.php` + `tests/Unit/Phase150/EtapaTransicaoServiceTest.php` + `tests/Feature/Phase34HubspotWebhookTest.php` (mock `Http::fake`) | exact (composto) |
...
$resultado = app(EtapaTransicaoService::class)->transicionar(
    ...
    $request->user(),   // NUNCA um user_id cru — sempre $request->user()/auth()->user() (T-150-02)
);
...
- `pendencia_fluxo` (a da Fase 150, declarada à mão — ver `app/Models/CompanyEtapaTransicao.php` / colunas de pendência criadas por `2026_09_01_130000_add_pendencia_to_companies_table.php`)
...
/** Fase 151 (COMERC-01/02/03) — módulo Entrada: casca da listagem de empresas em fluxo de entrada. */
...
### Ator "nunca aceitar user_id cru de payload/request" (T-150-02, reafirmado pela D-17)
...
```
**Apply to:** todo teste de `tests/Feature/Phase151/` que exercite o webhook ou `fetchOwner()` — adicionar rota fake `api.hubapi.com/crm/v3/owners/{id}*` à mesma chamada `Http::fake([...])`.
...
```
**Apply to:** `tests/Feature/Phase151/ComercContratoPermissaoPreservadaTest.php` (regressão, reafirma o mesmo teste após a mudança de `NAV_TREE`) e um teste irmão para `comercial.entrada.index` (`permission:comercial.entrada`, nunca `role:admin` nem `permission:comercial.cadastrar_empresa`).

### Teste unitário do serviço de transição — reler do banco, nunca confiar no objeto em memória
**Source:** `tests/Unit/Phase150/EtapaTransicaoServiceTest.php:18-70` (confirmado)
```php
// Source: tests/Unit/Phase150/EtapaTransicaoServiceTest.php:31-44
...
```
**Apply to:** `tests/Feature/Phase151/ComercEtapaNascimentoWebhookTest.php`/`ComercEtapaNascimentoCadastroManualTest.php` — não reabrir nem retestar `EtapaTransicaoService` em si (já coberto pela Fase 150); testar só o CALL SITE novo (webhook/store chamam o serviço, tratam `recusado`/`erro` sem quebrar).

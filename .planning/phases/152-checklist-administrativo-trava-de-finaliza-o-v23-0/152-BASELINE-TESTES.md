...
# Baseline de testes — Fase 152 (antes de qualquer código novo)

Coletada **antes** da primeira linha de código desta fase, conforme exigido por
`152-VALIDATION.md` § "Baseline obrigatória ANTES de qualquer código novo". Esta fase acrescenta
**novos chamadores** de `EtapaTransicaoService::transicionar()` (etapas 2/3/4/5, D-15) sobre uma
máquina de estados que já tem baseline própria (`150-BASELINE-TESTES.md`, `151-BASELINE-TESTES.md`).
Sem este documento não há como distinguir falha nova de falha herdada quando os planos 152-02 em
...
**Baseline verde.** As três suítes (`tests/Unit/Phase150`, `tests/Feature/Phase150`,
`tests/Feature/Phase151`) fecham **100% verdes** — 123 testes, 444 assertions, **nenhuma falha,
nenhum erro**. A ausência de falha é informação registrada explicitamente, não silêncio: qualquer
falha nova que aparecer a partir do plano 152-02 é regressão desta fase, não herança de 137/138.
...
e reconfirmadas em `151-BASELINE-TESTES.md` (6 falhas em `PolosControllerTest` por o cálculo de
...
fase — a Fase 152 não toca em nada do módulo Polos.
...
- **`php artisan test` e `php artisan test --testsuite=Feature` NÃO terminam.** Travam numa
  ...
  `150-BASELINE-TESTES.md` e `151-BASELINE-TESTES.md`, reconfirmado por `152-VALIDATION.md`. Não
...
Comparação de regressão nos planos 152-02 em diante é sempre contra este número:

| Suíte | Resultado baseline |
|---|---|
| `tests/Unit/Phase150` + `tests/Feature/Phase150` + `tests/Feature/Phase151` (comando único) | 0 falhas — **123/123 verde**, 444 assertions |

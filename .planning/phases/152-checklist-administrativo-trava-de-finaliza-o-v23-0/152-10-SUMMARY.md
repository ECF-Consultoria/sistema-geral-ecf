...
requires:
  ...
  - phase: 152-01
    provides: "152-BASELINE-TESTES.md — o 'antes' contra o qual a regressão final é comparada"
provides:
  - ".planning/phases/139-.../152-REGRESSAO-FINAL.md — o 'depois', com comparação nominal"
  - "Fase 152 COMPLETA: os 6 requirements ADMIN-01..06 fechados e marcados em REQUIREMENTS-v23.md"
...
# Phase 152 Plan 10: fechamento da fase Summary
...
|---|---|---|
| `tests/Unit/Phase152` + `tests/Feature/Phase152` | **92 testes / 364 assertions**, exit 0 | — (fase nova) |
| `Phase150` + `Phase151` + `Phase131` | **245 / 910**, exit 0 | a parte 137/138 fecha **123/444**, número por número **idêntica** |
...
`Phase131` entrou no comando porque o plano 152-08 tirou `admin.contratos.show` do grupo de permissão; os 122 testes daquela fase seguem verdes, `ContratoAdminPermissaoTest` incluído.
...
Detalhe em `152-REGRESSAO-FINAL.md`.
...
o que motivou `show()` a sincronizar (plano 152-08).
...
- **Saiu da listagem Entrada, continua na Contrato** — confirmado por consulta com o mesmo recorte
  de cada listagem. É a decisão D-06 da Fase 151 (a listagem Contrato **não** tem corte por etapa) se
...
- Próxima fase da milestone: **140** — a mensagem de boas-vindas. Ela segue dona do motor da
  ...
  separação de risco foi decidida na Fase 151 e continua valendo.

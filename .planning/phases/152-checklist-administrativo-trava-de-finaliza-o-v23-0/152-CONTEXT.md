# Phase 152: Checklist administrativo + trava de finalização (v23.0) - Context
...
**Fora do escopo:** o motor da mensagem de boas-vindas (Fase 153), a distribuição pela Coordenação
(Fase 154), o onboarding (142) e o histórico/SLA (143). Nada aqui reconstrói assinatura — o grupo
...
- **D-01:** **A lista que vale é a do §5 — 9 itens**, não os 12 do §3. Medida a diferença exata: o
  ...
  → Fecha a decisão que o `ROADMAP.md` marcou como **em aberto** no bloco da Fase 152.
...
- **D-04:** **"Gerar link de conexão com a ADMA" está errado em dois níveis.** Primeiro, o nome é
  ...
  **Onde mora:** `config/services.php`, como
  ...
  o mesmo padrão que a Fase 151 usou para as properties do HubSpot.
...
- **D-09:** **As permissões de módulo da Fase 151 bastam** — `comercial.entrada` para os itens de
  Entrada e para o FINALIZAR, `admin.contratos` para os itens de Contrato. **Nenhuma chave nova.**
  As duas já são liberáveis por setor sem deploy (D-15 da Fase 151).
...
As quatro decisões abaixo nasceram de conflitos que a `152-RESEARCH.md` mediu contra o código e
...
- **D-15:** **O progresso do checklist dirige as etapas 2, 3 e 4 — não o clique final.**
  ...
  **A régua:** 1º item concluído → `administrativo_andamento` (2); envelope Clicksign enviado →
  ...
  ⚠️ **O salto 2→4 já é previsto** na tabela da Fase 150 para empresa isenta de contrato (D-07):
  ...
  (`EtapaTransicaoService.php:28-32`) e dá à Fase 156 (HIST-03) o **instante real** de cada etapa.
...
- **D-18:** **Com 2+ envelopes ativos, manda o mais atrasado.**
  ...
  **A régua:** os itens 2 e 3 só fecham quando **TODOS** os envelopes ativos da empresa atingiram
  ...
  mesma regra para o onboarding da Fase 155 — *"Manda o onboarding mais atrasado — a empresa só
...
- `.planning/ROADMAP.md`, bloco `### Phase 152` — Goal, 5 Success Criteria, o lembrete D5 e o aviso
...
- `.planning/seeds/151-153-admin-no-comercial-dois-modulos-260902.md` — a fusão Estrutura+Comunicação
  em **Entrada**, e o mapa arquivo-a-arquivo do que a reorganização encosta.

### Decisões travadas nas fases anteriores

- `.planning/phases/151-rea-comercial-conectada-etapa-v23-0/151-CONTEXT.md` — D-02/D-03 (os dois
...
- `.planning/phases/151-rea-comercial-conectada-etapa-v23-0/151-VERIFICATION.md` — o que a Fase 151
  de fato entregou, verificado contra o código.
- `.planning/phases/151-rea-comercial-conectada-etapa-v23-0/138-CONTA-SISTEMA-HUBSPOT.md` — as 3
...
- `.planning/phases/151-rea-comercial-conectada-etapa-v23-0/151-VALIDATION.md` — **os comandos de
...
- **`app/Services/FluxoEntrada/EtapaTransicaoService`** (Fase 150) — **a única porta** para mudar
  `companies.etapa`. O FINALIZAR transiciona por ele, nunca por `update()` direto. A Fase 151
...
- `ComercialEntradaController` (Fase 151) — ganha a ação "Abrir" para a ficha (D-08).

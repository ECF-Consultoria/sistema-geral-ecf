...
# Phase 138 Plan 04: Owner e data da venda persistidos (webhook + retroativo) Summary
...
- Owner ausente (chave da property fora do payload), arquivado/removido (404) ou sem escopo OAuth
  ...
  regressão do próprio `HubspotOwnerResolver`/`fetchOwner()` (ambos já resilientes desde o 138-03).
...
- `tests/Feature/Phase138/ComercOwnerDataVendaPersistidosTest.php` - 4 testes: empresa nova,
...
- `app/Console/Commands/HubspotBackfillOwnerVenda.php` - comando `hubspot:backfill-owner-venda`,
  ...
  de estados
- `tests/Feature/Phase138/ComercBackfillOwnerVendaTest.php` - 6 testes: dry-run não grava, apply
...
real da property de owner (responsabilidade do plano 138-09, já registrada em
...
acervo antigo (comando manual, ainda não executado em produção). Os planos 138-05/138-06
...
completo neste plano — ele é multi-plano e só fecha quando as listagens (138-05/138-06)

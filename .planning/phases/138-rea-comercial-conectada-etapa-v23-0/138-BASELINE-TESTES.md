...
# Baseline de testes — Fase 138 (antes de qualquer migration)

Coletada **antes** de a migration do plano `138-03` existir. Aquela migration cria três colunas
...
o registro exigido por ele. Este plano (`138-01`) **não** modificou nenhum arquivo em `app/`,
...
comando ser rodado (medido também pelo `138-VALIDATION.md`, HEAD `2a9562af`). Não foi preciso
...
Comando:
```
C:\xampp\php\php.exe vendor/bin/phpunit tests/Feature/Phase37CompaniesPerformanceFilterTest.php tests/Feature/V16/CompanyControllerResponsavelPerformanceTest.php tests/Feature/Phase131/ContratoAdminPermissaoTest.php tests/Unit/Phase137 tests/Feature/Phase137 --colors=never
```

Resultado: **OK (86 tests, 214 assertions)** — código de saída 0.

Isolando as duas suítes de `/companies` sozinhas (o mesmo par que a Fase 137 mediu como
...
número da Fase 137. O total de 86/214 é maior porque este comando também soma
`tests/Feature/Phase131/ContratoAdminPermissaoTest.php`, `tests/Unit/Phase137` e
`tests/Feature/Phase137` — suítes que não existiam (ou não entravam neste comando) na baseline
da 137.
...
Comando:
```
C:\xampp\php\php.exe vendor/bin/phpunit tests/Feature/Phase131/ContratoAdminPermissaoTest.php tests/Unit/Phase137 --colors=never
```

Resultado: **OK (26 tests, 60 assertions)** — código de saída 0. Confirma que `admin.contratos.*`
segue sob `permission:admin.contratos` (D-15) e que a máquina de estados dos 9 status (Fase 137)
está verde antes de a Fase 138 tocar em qualquer coisa.
...
em `137-BASELINE-TESTES.md`: `test_meta_por_estagio`, `test_status_sim`,
...
Idêntico ao número da Fase 137: 2 `ArgumentCountError` em `SyncPolosFaturamentoJob::handle()`
...
Comparado à baseline da Fase 137 (285 testes, 987 assertions, mesmos 9 erros + 3 falhas), o total
de testes cresceu (+22 testes / +54 assertions) — atribuível aos testes Unit que a própria Fase
137 adicionou entre a baseline dela e hoje (`tests/Unit/Phase137/CompanyEtapaConstantesTest.php` e
`tests/Unit/Phase137/EtapaTransicaoServiceTest.php`, ambos incluídos na contagem `--testsuite=Unit`).
Os 9 erros e as 3 falhas são **exatamente os mesmos nominalmente** que a Fase 137 já havia
...
§2 e com o número que a Fase 137 já havia registrado. Causa raiz documentada lá, não é
...
(`Tests: X, Assertions: Y`). Achado herdado e reconfirmado do `137-BASELINE-TESTES.md`; não
foi repetido aqui para não queimar o orçamento de tempo desta task sem produzir número novo.

Todo comando desta fase (138-01 em diante) roda **sempre por diretório/arquivo**, com o binário
`C:\xampp\php\php.exe vendor/bin/phpunit` — nunca `php artisan test`, nunca `--testsuite=Feature`
sozinho. Ver `138-VALIDATION.md` § "Infraestrutura de Testes" para o comando rápido por task
(`tests/Unit/Phase138 tests/Feature/Phase138`) e o comando de suíte por fechamento de wave.

## Regra de comparação

Nenhuma falha listada acima é regressão da Fase 138 — este documento é a baseline "antes".
Comparação de regressão nos planos 138-02 em diante é sempre contra estes números, **nunca
contra zero**:

| Suíte | Resultado baseline |
|---|---|
| Comando 1 — Baseline específica da fase (companies + Contrato + Phase137) | 0 falhas — 86/86 verde, 214 assertions |

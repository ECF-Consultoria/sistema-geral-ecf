...
# Phase 138 Plan 08: Reorganização de navegação — Contrato e Entrada no Comercial Summary
...
## Accomplishments
- `Comercial/Entrada.jsx` deixou de ser a casca mínima do plano 138-05 e ganhou busca/ordenação server-side, badge de origem (HubSpot/Manual), badge de status do contrato, coluna de etapa (com "Sem etapa (legado)" para o legado nunca carimbado) e a coluna única de Pendências mostrando `pendencia_fluxo` e `pendencias_cadastro` como badges visualmente distintos, nunca somados (D-11)
...
- `resources/js/Pages/Comercial/Entrada.jsx` - componente completo: tabela dos 8 campos do §2, busca/ordenação server-side, badges de origem/contrato/pendências, texto informativo sem nenhum controle de checklist (Fase 139)
...
- A Fase 139 pode acrescentar o checklist dos 8 itens do módulo Entrada e os 4 do módulo Contrato sem qualquer dependência de navegação pendente — os dois módulos já são acessíveis pelo menu certo, com a permission certa.
...
- Nenhum arquivo de Contrato mudou de lugar — a Fase 139 pode continuar editando `ContratoAdminController.php`/`Admin/Contratos.jsx` nos mesmos caminhos de sempre.

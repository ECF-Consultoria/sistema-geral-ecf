...
provides:
  ...
  - "resources/js/Pages/Comercial/Entrada.jsx — página real (casca mínima), pré-requisito de runtime para o plano 138-08 estender"
affects: [138-08, 139]
...
key-files:
  created:
    ...
    - tests/Feature/Phase138/ComercEntradaPermissaoRotaTest.php
    - tests/Feature/Phase138/ComercListagemEntradaTest.php
    - tests/Feature/Phase138/ComercVisibilidadeAteEtapa5Test.php
  modified:
...
key-decisions:
  ...
  - "Criada resources/js/Pages/Comercial/Entrada.jsx como página REAL (não re-export) já na Task 1 — sem ela a rota nunca responde 200 (erro 'Unable to locate file in Vite manifest', o mesmo anti-padrão documentado em painel-polos-status-e-meta.md). O plano 138-08 é dono da UI completa (filtros, ações); esta versão é só a tabela dos 8 campos"
  - "COMERC-02 marcado como PARCIAL, não completo: o requisito exige as DUAS listagens (Contrato e Entrada) com os 8 campos. Este plano fecha só o lado Entrada — o lado Contrato é do plano 138-06. Só COMERC-03 foi marcado como fechado nesta execução"
...
# Phase 138 Plan 05: Módulo Comercial · Entrada (casca) Summary
...
- `tests/Feature/Phase138/ComercEntradaPermissaoRotaTest.php` - 7 casos de permissão/rota
- `tests/Feature/Phase138/ComercListagemEntradaTest.php` - 7 casos de payload/universo/pendências/setor
- `tests/Feature/Phase138/ComercVisibilidadeAteEtapa5Test.php` - 7 casos de fronteira (D-05/D-07/D-14)
...
- **Issue:** `Inertia::render('Comercial/Entrada', ...)` falhava com "Unable to locate file in Vite manifest: resources/js/Pages/Comercial/Entrada.jsx" — a página está formalmente atribuída ao plano 138-08, mas sem ela a rota NUNCA responde 200, nem em produção nem em teste. Mesmo anti-padrão documentado em `.planning/learnings/painel-polos-status-e-meta.md:83-88` (página nova como re-export puro some do manifest) — aqui o problema era pior: a página nem existia.
...
**Total deviations:** 2 auto-fixed (ambos Rule 3 - blocking, ambos necessários para a rota nova sequer responder). Nenhum scope creep: a UI completa (filtros, ações, colunas extras) continua sendo trabalho do plano 138-08; esta versão é deliberadamente mínima.
...
- O plano 138-06 pode acrescentar os 8 campos à listagem Contrato (`ContratoAdminController::index()`) sem qualquer dependência deste plano além do que a Fase 138-03 já entregou — os dois controllers são independentes.
- O plano 138-08 herda uma página `Entrada.jsx` funcional (não um re-export) para estender com filtros/ações; o payload já expõe `filters: {q, ordem}` para isso.

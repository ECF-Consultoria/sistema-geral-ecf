...
affects: [138-08, 138-09, 139, 141, 142, 143]
...
# Phase 138 Plan 07: Ator de sistema do webhook + nascimento na etapa 1 nas duas portas Summary
...
- Resolvido o único achado de risco ALTO do `138-RESEARCH.md`: não existia convenção de "ator de
...
- `config('services.hubspot.webhook_user_id')` (env `HUBSPOT_WEBHOOK_USER_ID`), **sem default** —
  ...
  configurada no `.env` deste worktree (não versionado); pendência explícita para o plano 138-09
...
- `EtapaTransicaoService` **não foi tocado** — D-17 proibia reabrir um serviço que a Fase 137
...
- 15 testes novos (`ComercAtorSistemaTest` 5, `ComercEtapaNascimentoWebhookTest` 6,
  ...
  `tests/Feature/Phase138` completa: 57/57 verde. Regressão: `tests/Unit/Phase137` +
  `tests/Feature/Phase137` + `Phase34HubspotWebhookTest` + `ContratoAdminPermissaoTest`: 68/68 verde.
...
- `.planning/phases/138-rea-comercial-conectada-etapa-v23-0/138-CONTA-SISTEMA-HUBSPOT.md` -
  registro por escrito (id local 49, `id_vps:` em branco para o plano 138-09)
...
- `tests/Feature/Phase138/ComercAtorSistemaTest.php` - 5 testes: dry-run, criação idempotente,
  dupla `--apply`, não-logabilidade (4 senhas), zero permission efetiva
- `tests/Feature/Phase138/ComercEtapaNascimentoWebhookTest.php` - 6 testes: nascimento no deal
...
- `tests/Feature/Phase138/ComercEtapaNascimentoCadastroManualTest.php` - 4 testes: etapa 1 no
...
conta ainda precisa ser criada na VPS antes do primeiro deploy da Fase 138 (pendência do plano
138-09, já registrada em `138-CONTA-SISTEMA-HUBSPOT.md`).
...
conta existir lá. Fica registrado como pendência explícita para o plano 138-09
...
caso. O plano 138-08 (navegação/`Entrada.jsx`) e o 138-09 (checkpoints humanos finais, incluindo a

...
**Esta fase entrega a CASCA.** As tarefas de checklist dentro dos módulos são da Fase 139.
...
Área Comercial
  ...
  └── Entrada                ← módulo NOVO (casca), recebe os 8 itens de checklist na Fase 139
```
...
- Webhook do HubSpot **e** cadastro manual do Comercial passam a nascer na etapa 1, chamando
  o `EtapaTransicaoService` da Fase 137.
...
- Mensagem de boas-vindas (Fase 140 — **cuja existência precisa ser reavaliada**, ver abaixo).
...
3. **Fase 139 no `ROADMAP.md`** repete os três grupos; **Fase 140** existe só para a mensagem
...
- **D-01:** A Fase 138 entrega **a casca**: reorganização da navegação + as duas listas com os
  8 campos do §2 + o nascimento na etapa 1. Os checklists de Contrato e Entrada continuam
  sendo a Fase 139 (ADMIN-01..06), que é onde já estavam. Motivo: misturar mudança de
...
- **D-06:** Na Fase 138, sem checklist ainda, cada lista nasce assim:
...
- **D-09:** **Onde grava** (discricionário — o usuário delegou): **colunas aditivas nullable**
  ...
  pega (`.planning/learnings/desempenho-bonificacao.md` §6), e a Fase 143 vai querer a data da
...
- **D-11:** **Duas pendências, em colunas separadas e nomeadas — nunca somadas.**
  ...
  Respeita a D-17 da Fase 137 (agregar as duas faria "pendência" significar o que esses
...
- **D-12:** **"Setor ou segmento" = setor ECF** (`servicos.setor`: performance/publicação/
  ...
  Comercial, e está sempre preenchido porque vem do serviço contratado. Na Fase 141 é esse
...
- **D-14:** **Legado com `etapa` NULL não é carimbado.** Segue a D-05 da Fase 137: nunca
  deduzir etapa a partir de estado externo, porque afirma um histórico que não aconteceu e
  vira duração fictícia no painel de SLA da Fase 143. As listas Contrato/Entrada nascem só com
...
- **D-09 (onde gravar owner/data da venda)** — o usuário respondeu "você decide". Decidido:
  ...
  que a Fase 143 não precisa da data como coluna.
...
- `.planning/seeds/138-140-admin-no-comercial-dois-modulos-260902.md` — **o enunciado na letra**,
...
- `.planning/ROADMAP.md` — "Phase 138" (~linha 2138) e o bloco de risco da milestone v23.0
...
- **`app/Services/FluxoEntrada/EtapaTransicaoService.php`** — pronto e sem chamador de
  ...
  risco de escrever o serviço do risco de trocar o caminho de produção). A Fase 138 é quem o
...
- **`app/Http/Controllers/ContratoAdminController.php`** + `Pages/Admin/Contratos.jsx` /
  `ContratoDetalhe.jsx` — o módulo Contrato inteiro, já funcional. A Fase 138 **move**, não
  reescreve.
...
- **Destino da Fase 140** — "Gerar mensagem de boas-vindas" virou item do módulo Entrada. A 140
...
- **§3 × §5 não batem em contagem** — o §3 lista 12 atividades, o §5 controla 9 com
  ...
  **decisão da Fase 139**, e precisa ser explícita, não deduzida.
- **Filtro por etapa nas listas do Comercial** — a Fase 137 entregou o filtro em `/companies`
  (ETAPA-05). Replicar nas listas novas não é exigido por COMERC-01/02/03.

### Reviewed Todos (not folded)

O `todo.match-phase 138` devolveu 5 candidatos, **nenhum dobrado**. Todos casaram por palavra

...
| 8-02-01 | 02 | 1 | (infra) | — | Enum `Categoria` retorna `'manual'` para `MANUAL->value` (round-trip via `Categoria::from`) | feature | (coberto por Test 7 / smoke) | ❌ W0 | ⬜ pending |
| 8-02-02 | 02 | 1 | (infra) | V5 Input Validation | `BaseNotification::toArray()` retorna 6 chaves canônicas; `via()` retorna `['database']` | feature | `php artisan test --filter=Phase8FoundationTest::test_base_notification_persiste_payload_canonico` | ❌ W0 | ⬜ pending |
| 8-03-01 | 03 | 2 | PERM-01 | V4 Access Control | `Permissions::all()` contém `'notificacoes.criar'`; `Permissions::NOTIFICACOES_CRIAR` é a constante | feature | `php artisan test --filter=Phase8FoundationTest::test_permissions_all_inclui_notificacoes_criar` | ❌ W0 | ⬜ pending |
| 8-03-02 | 03 | 2 | PERM-01 | V4 Access Control | `Permissions::catalog()['Notificações']` existe com label `'Criar notificações'` e descrição pt-BR mencionando "manuais" | feature | `php artisan test --filter=Phase8FoundationTest::test_catalog_inclui_grupo_notificacoes` | ❌ W0 | ⬜ pending |
| 8-03-03 | 03 | 2 | PERM-03 | V4 Access Control | `Permissions::AUTO_LIDERANCA` contém `'notificacoes.criar'` | feature | `php artisan test --filter=Phase8FoundationTest::test_auto_lideranca_inclui_notificacoes_criar` | ❌ W0 | ⬜ pending |
| 8-04-01 | 04 | 2 | PERM-02 | V4 Access Control | Admin (`role=admin`) retorna `true` em `hasPermission('notificacoes.criar')` sem atribuição de setor nem permissão manual | feature | `php artisan test --filter=Phase8FoundationTest::test_admin_tem_permissao_via_short_circuit` | ❌ W0 | ⬜ pending |
| 8-04-02 | 04 | 2 | PERM-03 | V4 Access Control | User não-admin com `setoresLiderados()` ativo retorna `true` em `hasPermission('notificacoes.criar')` sem permissão manual no setor | feature | `php artisan test --filter=Phase8FoundationTest::test_lider_tem_permissao_via_auto_lideranca` | ❌ W0 | ⬜ pending |
| 8-04-03 | 04 | 2 | (infra) | — | `Notification::send($user, new TestAnon())` persiste 1 linha em `notifications` com `data` contendo as 6 chaves canônicas (titulo, mensagem, categoria, autor_user_id, url, meta) | feature | `php artisan test --filter=Phase8FoundationTest::test_base_notification_persiste_payload_canonico` | ❌ W0 | ⬜ pending |

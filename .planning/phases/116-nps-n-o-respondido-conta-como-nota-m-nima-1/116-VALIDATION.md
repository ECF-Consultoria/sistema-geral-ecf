...
| NPSFLOOR-08c | Reconsolidação **recusada pelo gate de margem FIXMARG-03** não passa por sucesso: o comando re-consulta o `DesempenhoScoreSnapshot` das pessoas/competências do relatório aprovado e **nomeia** (nome + competência) quem não foi atualizado, com exit code de falha | Feature (Console) | `php artisan test --filter=NpsMaterializarNaoRespondidosCommandTest` (caso `degradad*`, com `expectsOutputToContain($user->name)`) | ❌ Wave 0 |
...
- [ ] `tests/Feature/Phase116/NpsMaterializarNaoRespondidosCommandTest.php` — cobre NPSFLOOR-08, **NPSFLOOR-08b e NPSFLOOR-08c** (fixture de snapshot no molde de `tests/Feature/Phase74/ConsolidarMesDesempenhoCommandTest.php`, única do repo que faz `desempenho:consolidar-mes` gravar snapshot em SQLite; fixture de margem degradada no molde de `tests/Feature/Phase110/ConsolidarMesMargemResilienteTest.php`, que força `cobertura < 0,7` via provider stub)
...
| Impacto aprovado aparece de fato nas telas de mês fechado | NPSFLOOR-08b / NPSFLOOR-08c / D1 | Só verificável contra dados reais depois de aplicar | Depois do backfill: ler o bloco de **conferência** impresso pelo comando (`Conferência OK` ou a tabela nomeando pessoa + competência não atualizadas) e conferir ranking de `/performance`, Relatório de Bonificação e auditoria de bônus da competência aplicada; havendo nome na lista de conferência, re-rodar `desempenho:consolidar-mes --mes=YYYY-MM` e depois `nps:materializar-nao-respondidos --mes=YYYY-MM --force` até a conferência sair OK (a contagem `Degradados` do consolidar-mes NÃO nomeia ninguém) |

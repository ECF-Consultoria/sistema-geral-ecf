...
provides:
  ...
  - "150-BASELINE-TESTES.md — baseline verde registrada antes de qualquer migration da fase"
  - "Achado documentado: suíte Feature completa não termina neste ambiente offline (timeout de rede em cascata)"
affects: [150-02, 150-03, 150-04, 150-05, 150-06, 150-07, gsd:verify-work]
...
key-decisions:
  - "Usado o caminho definitivo npm install + npm run build (não o atalho public/hot) — ambiente fica destravado de forma persistente para os planos 150-02..07, sem precisar recriar public/hot a cada task"
...
patterns-established:
  - "150-BASELINE-TESTES.md é o documento 'antes' — comparação de regressão nos planos seguintes sempre contra os números ali registrados, nunca contra zero"
...
# Phase 150 Plan 01: Destravar ambiente de teste + baseline pré-migration Summary

**Ambiente Inertia/Feature-test destravado via `npm install && npm run build` (não o atalho `public/hot`); baseline verde de 24/44 testes registrada em `150-BASELINE-TESTES.md` antes de qualquer migration, com as falhas antigas de Polos (10) e um achado novo — cascata de timeout de rede que impede rodar a suíte Feature completa neste ambiente offline — nomeados por escrito.**
...
- `public/build/manifest.json` existe — toda Feature test que renderiza Inertia passa sem
  ...
  26 assertions, verde).
- `150-BASELINE-TESTES.md` criado com: comando exato de cada execução, contagem literal
...
registrado dentro do `150-BASELINE-TESTES.md` da Task 2.
...
- `.planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-BASELINE-TESTES.md` — baseline de testes pré-migration, com achados de ambiente e falhas pré-existentes nomeadas
...
- **Caminho `npm-build` em vez de `public/hot`:** o plano permitia os dois; `npm install` e
  ...
  `npm run build` quando o plano 150-07 alterar `Companies/Index.jsx`.
...
- **Suíte Feature completa (`--testsuite=Feature`) não termina neste ambiente.** Trava numa
  ...
  task. Documentado em detalhe no `150-BASELINE-TESTES.md`, com recomendação para as próximas
...
- Ambiente de teste Feature/Inertia está pronto para os planos 150-02..07 rodarem suas próprias
  suítes (`tests/Unit/Phase150`, `tests/Feature/Phase150`) sem bloqueio de manifest do Vite.
- Baseline registrada e datada ANTES da migration — 150-02 pode prosseguir com a migration de
  `companies.etapa` sabendo exatamente quais falhas já existiam antes dela.
- **Atenção para 150-02..07 e para `/gsd:verify-work`:** não tentar `php artisan test`
  completo esperando resultado — vai travar na mesma cascata de rede. Usar os comandos
  registrados em `150-BASELINE-TESTES.md` (`Quick run` por task, `Baseline command` por wave).
- Nenhum bloqueio para o plano 150-02.
...
- FOUND: `.planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-BASELINE-TESTES.md`
- FOUND: `.planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-01-SUMMARY.md`
- FOUND commit: `96e62ed9` (docs(150-01): registra baseline de testes antes da migration da fase)
- FOUND commit: `4429af7e` (docs(150-01): SUMMARY do plano 01)

...
provas de `tests/{Unit,Feature}/Phase150` passam localmente (verificado nesta
revisão).
...
**File:** `tests/Feature/Phase150/EtapaPontoUnicoTest.php:270-300`
**Issue:**
...
desalinhado com o propósito do dado" que a Fase 156 usa para medir SLA. O
...
### WR-01: `EtapaTransicaoService::transicionar()` decide e escreve sem lock — race condition latente para quando a Fase 151 conectar chamadores concorrentes (ex.: webhook)
...
não há chamador de produção nesta fase. Mas a Fase 151 (webhook → etapa 1) é
...
de `etapa` como fonte (Fase 155), mas é real e silencioso.
...
`tests/Feature/Phase150/EtapaFiltroListagemTest.php`) — a limitação é

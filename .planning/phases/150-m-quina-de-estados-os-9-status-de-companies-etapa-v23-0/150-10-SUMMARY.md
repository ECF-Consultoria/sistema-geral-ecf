...
provides:
  - "`EtapaTransicaoService::transicionar()` releva e trava a linha (`lockForUpdate()`) DENTRO da transação, decidindo sobre o estado do banco, nunca sobre o objeto `Company` em memória (fecha WR-01/T-150-31/T-150-32)"
  - "`etapa:backfill` chaveia o balde 1 por `id` (único), imune a colisão de `companies.name` (fecha WR-02/T-150-33)"
  - "`150-01-PLAN.md` não reivindica mais `ETAPA-01`/`ETAPA-02` — traceability honesta (fecha WR sem código, T-150-35)"
...
key-files:
  ...
  modified:
    ...
    - .planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-01-PLAN.md
...
key-decisions:
  ...
  - "150-01-PLAN.md ganhou requirements: [] + comentário YAML de uma linha, sem tocar em must_haves/tasks/corpo — registro histórico do que foi executado permanece intacto"
...
# Phase 150 Plan 10: Gap closure — TOCTOU no lock, homônimo no backfill, frontmatter honesto Summary

**`EtapaTransicaoService::transicionar()` passou a decidir sobre a linha travada por `lockForUpdate()` dentro da transação (não mais sobre o `Company` em memória do chamador); `etapa:backfill` chaveia o balde 1 por `id` em vez de `name` (não único); `150-01-PLAN.md` deixou de reivindicar `ETAPA-01`/`ETAPA-02` que não entregou.**
...
- **G6 fechado:** `150-01-PLAN.md` teve `requirements: [ETAPA-01, ETAPA-02]` trocado por `requirements: []` com um comentário de uma linha explicando o motivo — o plano só destravou o ambiente de teste, sem tocar `app/`/`database/`. `ETAPA-01` continua coberto por `150-02`, `ETAPA-02` por `150-05`/`150-10` — nenhum dos 6 IDs (`ETAPA-01`..`06`) ficou órfão (conferido por grep em todos os 11 planos).
...
3. **Task 3: Corrigir o frontmatter de 150-01-PLAN.md (G6)** - `9eaa4fac` (docs)
...
- `tests/Unit/Phase150/EtapaTransicaoServiceTest.php` - 4 testes novos: divergência memória-vs-banco (recusa e `etapa_anterior` correta), sincronização do objeto do chamador, erro sem exceção em empresa removida
- `app/Console/Commands/EtapaBackfill.php` - balde 1 chaveado por `id`; amostra do dry-run buscada à parte
- `tests/Feature/Phase150/EtapaBackfillTest.php` - 1 teste novo: duas empresas com `name` idêntico, ambas recebem `em_operacao`
- `.planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-01-PLAN.md` - `requirements: []` + comentário de justificativa
...
Os 3 achados do `150-REVIEW.md` (G4/WR-01, G5/WR-02) e o defeito de frontmatter (G6) estão fechados. Falta apenas **150-11 (G3)** entre os planos de gap closure planejados para esta fase. `EtapaTransicaoService` segue sem chamador de produção, de propósito — a Fase 151 (webhook Clicksign → etapa 1) é quem vai exercitar o lock sob concorrência real pela primeira vez; o teste desta fase prova a re-leitura, não a serialização (SQLite não permite provar isso).
...
- FOUND: `tests/Unit/Phase150/EtapaTransicaoServiceTest.php`
- FOUND: `tests/Feature/Phase150/EtapaBackfillTest.php`
- FOUND: `.planning/phases/150-m-quina-de-estados-os-9-status-de-companies-etapa-v23-0/150-01-PLAN.md`

...
| 10 | Granularidade da consulta de envelope (suficiente para reconciliação) | REDE-04 | ✅ suficiente **pela forma da API** — `status` + `meta.record_count` + eventos paginados. ✅ **CONFIRMADO end-to-end em 2026-08-18**, na virada da Fase 132 (ver `132-GATE.md`, seção do gate #10). Envelope de produção **realmente assinado** (3 assinaturas), aviso automático desligado de propósito por `PATCH /webhooks/{id}` → `inactive`, envelope fechado, e a contagem de eventos ficou em **25 do começo ao fim** — nenhum evento de fechamento entrou. `php artisan clicksign:reconciliar` percebeu sozinho: contrato `aguardando_assinaturas` → `assinado` e `ContratoLiberacao id=1` com **`via=reconciliacao`**, que é o carimbo que prova quem corrigiu. Veredito: **SUFICIENTE**. A Fase 130 não conseguira fazer isso porque a sandbox não conclui assinatura. ⚠️ O rate limit **20/min é o da Clicksign**; o bucket `clicksign-webhook` do próprio app é **3/min GLOBAL** (`AppServiceProvider.php`), e é ele que dita o desenho da varredura (1 job por contrato, nunca laço HTTP no comando) |

...
| 260618-n25 | Estende o modo Individual da Precificação (m0g) p/ MC e LL também por produto. O toggle (rótulo "Imposto:"→"Modo:") agora cobre imposto + MC + LL; chave persistida `modo_imposto` mantida. Helpers `mcEfetivo(row)`/`llEfetivo(row)`: no individual, campo `mc_individual`/`ll_individual` preenchido vira override do produto; VAZIO HERDA o global (mcNum/llNum) — o "padrão" (difere do imposto, onde vazio=0). Lote ganha colunas "MC %"/"LL %" (só no individual, grupo de produto, headerGroups span 3↔6); os 8 computes usam mcEfetivo/llEfetivo. Simulador: "Seus alvos" edita mc_individual/ll_individual do produto no individual (placeholder = global) e o global no massa; chips refletem o produto. Comissão e Acréscimo seguem globais; regressão zero no massa; sem migration/backend. | 2026-06-18 | 223a15b, df42bbe | [260618-n25-mc-e-ll-individuais-por-produto-no-modo-](.planning/quick/260618-n25-mc-e-ll-individuais-por-produto-no-modo-/) |

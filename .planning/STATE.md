...
Last activity: 2026-06-19 — Completed quick task 260619-dce: gráfico rose (RosePie) na distribuição de faturamento de /polos
...
| 260619-dce | Gráfico /polos "Distribuição do faturamento": troca a pizza 3D (`Pie3D`) por uma pizza **nightingale rose** em SVG puro (novo componente `RosePie.jsx`) — raio ∝ valor, fatias ordenadas, fundo escuro/glow via `feGaussianBlur`, leader lines com polo+R$+%, multicor pela `POLO_PALETTE` (SEM ECharts/recharts, zero nova dependência). Troca cirúrgica: pizza de STATUS e DonutCards intactos, `Pie3D` preservado (ainda usado no status). `npm run build` verde. Nota: HEAD detached acidental p/ origin/main durante o planejamento foi recuperado (cherry-pick p/ deploy/precificacao-onboarding). | 2026-06-19 | 28bf67d | [260619-dce-grafico-rose-faturamento-polos](.planning/quick/260619-dce-grafico-rose-faturamento-polos/) |

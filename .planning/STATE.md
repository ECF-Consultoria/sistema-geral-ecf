...
Last activity: 2026-06-26 — Quick 260626-ddp: Fechamento (Administrativo) passa a agrupar pelos grupos do Comercial (CompanyGroup) — linha única por grupo + toggle de recebido do grupo; remove o montar-grupos do admin (aba "Grupos"); migration zera parent_company_id; build verde; sem deploy
...
- 2026-06-26 — **Phase 38: Painel do Publicador** adicionada. Evoluir o "Meu Painel" do publicador (`/mlb/meu-painel` → `MlbController@meuPainel` → `Mlb/MeuPainel.jsx`) para o formato visual da Carteira dos analistas de ADS (`Portfolio/Show.jsx` + `PortfolioScoreService`): score 0–100, radar de 5 eixos (Meta · Produtividade · Pontualidade · Conversão · Qualidade), KPIs grandes (Faturamento `net_billing` · Anúncios Feitos · Vendas no Mês) e evolução do faturamento. Decisões travadas via AskUserQuestion (CONTEXT.md): evoluir tela existente (não criar separada); visão só individual (sem consolidada admin/líder); sem migration (dados já em `mlb_publicacoes`/`mlb_empresas`/`mlb_meta_historico`); mantém rota e permissão `mlb.meu_painel`. Próximo: `/gsd-plan-phase 38`.
...
| 260626-ddp | Fechamento (Administrativo) passa a agrupar pelos GRUPOS do Comercial (`CompanyGroup`/`company_group_id`) em vez de pai/filhas (`parent_company_id`). Cada grupo vira UMA linha consolidada (nome+cor, "Grupo · N", soma faturamento/cobrança) com um único toggle de "recebido" do grupo (`toggleRecebidoGrupo` → rota `admin.financeiro.grupo.recebido`, marca/desmarca todos os membros no mês); expansão mostra a composição com gestão de contrato por membro. Empresas sem grupo = linhas avulsas; `TotalConsolidado` conta 1x/linha; gráficos achatam membros (`empresasFlat`). Remove a aba "Grupos" (GruposManager) de `Admin/Empresas.jsx` — grupos só se definem no Comercial; `empresas()` para de enviar `grupos`/`servicos_disponiveis`. Migration `2026_06_26_123900` zera `parent_company_id` (coluna preservada; `down()` no-op). Follow-up: regrupar os PDFs/email por grupo (degradam graciosamente p/ 1 empresa/bloco após limpar parent). `npm run build` verde; migration NÃO rodada local (há 4 pendentes de outros trabalhos); sem deploy. | 2026-06-26 | 6f2863e, 325b34e, 996f6d3, 1d3f43c | [260626-ddp-fechamento-grupos-comercial](.planning/quick/260626-ddp-fechamento-grupos-comercial/) |

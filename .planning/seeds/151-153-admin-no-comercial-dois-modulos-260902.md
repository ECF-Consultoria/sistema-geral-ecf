...
**Origem:** decisão trazida pelo usuário durante `/gsd-discuss-phase 151`, repassada a ele
...
2. **Fase 152 no `ROADMAP.md`** repete "os 9 itens do §5" e a divisão em três grupos.
3. **Fase 153** existe só para a mensagem de boas-vindas (COMUNIC-01/02/03). "Gerar mensagem de
...
- **Fase 150 inteira** — a máquina de estados não é tocada. `EtapaTransicaoService` segue em
...
## Decisões da Fase 151 já fechadas
...
- **Onde grava** (discricionário, delegado ao Claude): colunas aditivas nullable
  ...
  `WHERE`/`ORDER` no MariaDB é armadilha que o SQLite dos testes não pega, e a Fase 156 vai
